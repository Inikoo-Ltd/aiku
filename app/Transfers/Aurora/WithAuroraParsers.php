...
trait WithAuroraParsers
{
    ...
    protected function parseLanguageID($locale): ?int
    {
        if ($locale != '') {
            try {
    ...
    protected function parseCurrencyID($currencyCode): ?int
    {
        if ($currencyCode != '') {
            if ($currencyCode == 'LEU') {
                $currencyCode = 'RON';
    ...
    protected function parseTimezoneID($timezone): ?int
    {
        if ($timezone != '') {
    ...
    protected function parseCountryID($country, $source = ''): ?int
    {
        if ($country != '') {
            try {
                if (strlen($country) == 2) {
    ...
    public function parseWebpage($sourceId): ?Webpage
    {
    ...
    public function parseHistoricAsset($organisation, $productKey): ?HistoricAsset
    {
    ...
    public function parseHistoricSupplierProduct($organisationID, $productKey): ?HistoricSupplierProduct
    {
    ...
    public function parseBarcode($sourceId): ?Barcode
    {
