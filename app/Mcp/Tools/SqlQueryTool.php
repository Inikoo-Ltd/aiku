...
#[Description('Run a read-only SQL SELECT against the Aiku PostgreSQL database, or against the NightOwl telemetry database (requests, exceptions, queries, jobs, logs) by passing database: nightowl, or against the archive database by passing database: archive. Only available to users with SQL access enabled. Use describe-tables-tool to discover schema. Always add your own LIMIT. Gotchas: never guess the values of enum-like columns (type, state, status) — describe-tables-tool reports their actual values; the legacy *_intervals tables were dropped — use *_time_series and *_time_series_records instead; filter suppliers by country via their address country_id, not by matching location text; text columns use a nondeterministic collation so ILIKE and regex fail — write col COLLATE "C" ILIKE ... instead. The ? character is read as a query placeholder, so the jsonb ? operator fails: write jsonb_exists(col, \'key\') instead. Per SKU stock history older than the retention window lives only in the archive: org_stock_histories and location_org_stock_histories keep the last three years daily plus one snapshot per month before that, and every other day is in database: archive with the same columns — query both and union when a question spans years. Stock money columns (org_stocks.value_in_locations and similar) are in each organisation\'s own currency (organisations.currency_id: GBP, EUR, IDR, USD...): never sum them across organisations, report per organisation. For discontinue or clearance questions prefer org-stock-sales-tool with sort worst per organisation, and include discontinuing SKUs when counting sales, they still sell. Read the aiku data guide resource before writing revenue or stock history queries.')]
#[IsReadOnly]
class SqlQueryTool extends Tool
{
    ...
    public function handle(Request $request): Response
    {
        ...
        return Response::json(array_filter([
            ...
            'hint'      => $this->orgStockWriteToolsHint($request, $sql),
        ], fn ($value) => $value !== null));
    }
