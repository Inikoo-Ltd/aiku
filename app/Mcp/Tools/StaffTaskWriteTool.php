...
#[Description('Ask a colleague or a department to do something: creates a staff task (TASK-n) with its own chat thread, requested by the authenticated user. Use it for work people do, e.g. ask the warehouse to count a location, ask buying to chase a supplier. Give either an assignee (aiku username) or a department slug (e.g. warehouse); an unknown department lists the valid ones. Optionally link the task to a location or SKO in an organisation so it shows where the work is. Tickets are different: bugs and requests to engineers go through tickets, and engineers and QA cannot be given tasks. Returns the task reference and its URL.')]
class StaffTaskWriteTool extends Tool
{
    public function handle(Request $request): Response
    {
        ...
        if ($request->filled('assignee')) {
            $assignee = $user->group->users()->where('username', $request->string('assignee')->lower()->toString())->first();
            if (!$assignee) {
    ...
    {
        return [
            'subject'      => $schema->string()->description('What needs doing, e.g. "Count location 45W24"')->required(),
            'description'  => $schema->string()->description('Details and why, posted as the first message in the task thread (max 5000 characters)'),
            'assignee'     => $schema->string()->description('aiku username of the colleague to do it. Give this or department'),
