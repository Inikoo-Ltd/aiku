...
trait WithDownloadPortfolios
{
    ...
    /**
     * @return array{0: string, 1: mixed, 2: mixed, 3: array, 4: bool}
     */
    protected function getDownloadPortfoliosInput($request): array
    {
        return [
            (string)$request->input('type', 'portfolio_csv'),
            $request->input('columns'),
            $request->input('product_states'),
            $request->input('product_availability') ?? [],
            $request->boolean('include_bundles'),
        ];
    }
}
