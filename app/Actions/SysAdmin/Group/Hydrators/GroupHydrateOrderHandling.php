...
class GroupHydrateOrderHandling implements ShouldBeUnique
{
    ...
    public function handle(int $groupID): void
    {
        ...
        $stats = [
            ...
            'number_orders_state_submitted_paid'                    => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->whereColumn('payment_amount', '>=', 'total_amount')->count(),
            'orders_state_submitted_paid_amount_grp_currency'       => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->whereColumn('payment_amount', '>=', 'total_amount')->sum('grp_net_amount'),

            'number_orders_state_submitted_not_paid'                => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->where('payment_amount', 0)->count(),
            'orders_state_submitted_not_paid_amount_grp_currency'   => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->where('payment_amount', 0)->sum('grp_net_amount'),
