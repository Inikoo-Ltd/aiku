...
class GroupHydrateOrderHandling implements ShouldBeUnique
{
    ...
    public string $commandSignature = 'hydrate:group-order-handling {groupID}';
    ...
    {
        $groupID = $command->argument('groupID');
    ...
    public function handle(int $groupID): void
    {
        ...
        $stats = [
            ...
            'number_orders_state_submitted_paid'                    => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->whereIn('pay_status', [OrderPayStatusEnum::PAID, OrderPayStatusEnum::NO_NEED])->count(),
            'orders_state_submitted_paid_amount_grp_currency'       => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->whereIn('pay_status', [OrderPayStatusEnum::PAID, OrderPayStatusEnum::NO_NEED])->sum('grp_net_amount'),

            'number_orders_state_submitted_not_paid'                => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->whereIn('pay_status', [OrderPayStatusEnum::UNPAID, OrderPayStatusEnum::UNKNOWN])->count(),
            'orders_state_submitted_not_paid_amount_grp_currency'   => $group->orders()->where('state', OrderStateEnum::SUBMITTED)->whereIn('pay_status', [OrderPayStatusEnum::UNPAID, OrderPayStatusEnum::UNKNOWN])->sum('grp_net_amount'),
