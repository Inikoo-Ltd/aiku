...
/**
 * Second step: hands back the code Meta sent to the number, which marks it as verified.
 * A verified number is still not able to send until it is registered.
 */
class VerifyWhatsappPhoneNumberCode extends OrgAction
{
     ...
     */
    public function handle(Shop $shop, string $code): array
    {
        [
        ...
        $response = Http::withToken($accessToken)->asForm()->post(
            $this->whatsappEndpoint($phoneNumberId.'/verify_code'),
            ['code' => $code]
        );

        if ($response->failed()) {
            return $this->graphFailure($response, __('Meta rejected the verification code.'));
        }
