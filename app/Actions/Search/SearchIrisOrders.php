...
class SearchIrisOrders
{
     ...
     * references only, no boosts and no synonyms, an order number is an identifier
     * rather than a phrase. Baskets and cancelled orders are filtered out at the
     * index so they do not eat into the hits limit.
     *
    ...
    public static function searchParameters(string $query, int $customerId, ?int $shopId): array
    {
        ...
        return array_merge(
            [
                ...
                'q'                      => $query,
                'query_by'               => config('scout.typesense.model-settings.'.Order::class.'.search-parameters.query_by') ?? 'reference,customer_reference',
