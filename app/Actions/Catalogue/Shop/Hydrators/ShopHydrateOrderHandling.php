...
class ShopHydrateOrderHandling implements ShouldBeUnique
{
    ...
    public function getMigrationStatsHack(Shop $shop): array
    {
        ...
        return [
            ...
            'number_orders_state_submitted_not_paid'              => $shop->orders()->whereNull('source_id')->where('state', OrderStateEnum::SUBMITTED)->where('pay_status', OrderPayStatusEnum::UNPAID)->count(),
            'orders_state_submitted_not_paid_amount'              => $shop->orders()->whereNull('source_id')->where('state', OrderStateEnum::SUBMITTED)->where('pay_status', OrderPayStatusEnum::UNPAID)->sum('net_amount'),
            'orders_state_submitted_not_paid_amount_org_currency' => $shop->orders()->whereNull('source_id')->where('state', OrderStateEnum::SUBMITTED)->where('pay_status', OrderPayStatusEnum::UNPAID)->sum('org_net_amount'),
            'orders_state_submitted_not_paid_amount_grp_currency' => $shop->orders()->whereNull('source_id')->where('state', OrderStateEnum::SUBMITTED)->where('pay_status', OrderPayStatusEnum::UNPAID)->sum('grp_net_amount'),
