...
class AssetHydrateInvoicedCustomersIntervals implements ShouldBeUnique
{
    use AsAction;
    ...
    public function handle(int $assetID, ?array $intervals = null, ?array $doPreviousPeriods = null): void
    {
        ...
        $stats     = [];
        $queryBase = DB::table('transactions')->whereNull('deleted_at')->where('asset_id', $asset->id)->selectRaw('count(distinct customer_id) as  sum_aggregate  ');

        $stats     = $this->getIntervalsData(
            stats: $stats,
            queryBase: $queryBase,
            statField: 'customers_invoiced_',
            intervals: $intervals,
