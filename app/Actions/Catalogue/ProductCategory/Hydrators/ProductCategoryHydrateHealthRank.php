...
class ProductCategoryHydrateHealthRank implements ShouldBeUnique
{
    use AsAction;

    public string $commandSignature = 'hydrate:product-category-health-rank';

    public function asCommand(Command $command): void
    {
        $this->handle();
        $command->info('Product category health ranks updated.');
    }

    public function handle(): void
    {
        $this->rankByType('family', 'family_id');
        $this->rankByType('sub_department', 'sub_department_id');
        $this->rankByType('department', 'department_id');
    }
    ...
    private function rankByType(string $type, string $fkColumn): void
    {
        DB::statement("
            ...
            FROM final_ranks fr
            WHERE product_categories.id = fr.category_id
              AND product_categories.type = '{$type}'
        ");
        ...
        DB::statement("
            ...
            WHERE type = '{$type}'
              AND NOT EXISTS (
                ...
                WHERE it.{$fkColumn} = product_categories.id
