...
class CollectionHydrateOrderingIntervals implements ShouldBeUnique
{
    use AsAction;
    ...
    public string $commandSignature = 'hydrate:collection-ordering-intervals {shop}';
    ...
    public function handle(int $collectionId, ?array $intervals = null, ?array $doPreviousPeriods = null): void
    {
        ...
        if ($assetIds->isEmpty()) {
            $collection->orderingIntervals()->update([]);
            return;
        }
        ...
        $collection->orderingIntervals()->update($stats);
    }
}
