...
class IndexOrdersInOrganisation extends OrgAction
{
    use WithGroupOverviewAuthorisation;

    protected function getElementGroup(Organisation $organisation): array
    {
        return [
            'state' => [
                'label'    => __('State'),
                'elements' => array_merge_recursive(
                    ...
                    OrderStateEnum::count($organisation)
                ),
    ...
    public function handle(Organisation $organisation, $prefix = null): LengthAwarePaginator
    {
        ...
        foreach ($this->getElementGroup($organisation) as $key => $elementOrganisation) {
            $query->whereElementGroup(
                key: $key,
                allowedElements: array_keys($elementOrganisation['elements']),
                engine: $elementOrganisation['engine'],
                prefix: $prefix
            );
        }
    ...
    public function tableStructure(Organisation $organisation, $prefix = null): Closure
    {
        return function (InertiaTable $table) use ($organisation, $prefix) {
            ...
            foreach ($this->getElementGroup($organisation) as $key => $elementOrganisation) {
                $table->elementGroup(
                    key: $key,
                    label: $elementOrganisation['label'],
                    elements: $elementOrganisation['elements']
                );
            }
    ...
    public function asController(Organisation $organisation, ActionRequest $request): LengthAwarePaginator
    {
        ...
        return $this->handle(organisation: $organisation, prefix: OrdersTabsEnum::ORDERS->value);
    }
