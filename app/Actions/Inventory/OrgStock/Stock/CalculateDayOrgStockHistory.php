...
class CalculateDayOrgStockHistory implements ShouldBeUnique
{
    ...
    public function handle(?int $orgStockId, Carbon $date, ?Command $command = null): void
    {
        ...
        $isCurrent = DB::connection('aiku:read')->table('location_org_stocks')->where('org_stock_id', $orgStock->id)->exists();
        if ($isCurrent) {
    ...
    {
        $rawDate = DB::connection('aiku:read')->table('org_stock_movements')->select('date')->where('org_stock_id', $orgStock->id)
