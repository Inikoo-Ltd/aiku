...
class IndexMasterProductsWithNoImage extends OrgAction
{
    ...
    public function handle(Group|MasterShop|MasterProductCategory $parent, $prefix = null): LengthAwarePaginator
    {
        ...
        $queryBuilder->whereNull('master_assets.image_id');
    ...
    {
        $title = __('Master Products with No Image');
        ...
        if ($this->parent instanceof MasterShop) {
            ...
            $afterTitle    = [
                'label' => __('Master Products') . ' ('.__('Missing Image').')'
            ];
            $iconRight     = [
                'icon' => 'fal fa-cube',
            ];
    ...
    public function getBreadcrumbs(Group|MasterShop|MasterProductCategory $parent, string $routeName, array $routeParameters, ?string $suffix = null): array
    {
        ...
        return match ($routeName) {
            'grp.masters.master_shops.show.master_products_no_image' =>
            array_merge(
