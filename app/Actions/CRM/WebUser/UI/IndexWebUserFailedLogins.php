...
class IndexWebUserFailedLogins
{
    use AsAction;
    ...
    public function handle(WebUser $webUser, ?string $prefix = null): LengthAwarePaginator
    {
        ...
        return QueryBuilder::for(WebUserFailedLogin::class)
            ->where('web_user_id', $webUser->id)
            ->defaultSort('-failed_at')
            ->allowedSorts(['failed_at', 'source', 'ip_address'])
    ...
    public function tableStructure(?string $prefix = null): Closure
    {
        return function (InertiaTable $table) use ($prefix) {
            ...
            $table
                ->withEmptyState([
                    'title' => __('No failed logins recorded yet'),
                    'count' => 0,
                ])
                ->column(key: 'failed_at', label: __('Date'), canBeHidden: false, sortable: true, type: 'date_hms')
                ->column(key: 'username', label: __('Username'), canBeHidden: false)
