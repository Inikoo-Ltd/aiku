...
class RunProspectMailshotSecondWave
{
    use AsAction;
    public string $jobQueue = 'default-long';
    public string $commandSignature = 'run-prospect-mailshot-second-wave';

    public function handle(): void
    {
        $currentDateTime = Carbon::now()->utc();

        // use mailshot second wave filter for prospect mailshots
        $secondWaveQuery = QueryBuilder::for(Mailshot::class);
        $secondWaveQuery->where('type', MailshotTypeEnum::INVITE);
