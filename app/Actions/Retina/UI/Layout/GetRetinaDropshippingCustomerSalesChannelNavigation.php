...
class GetRetinaDropshippingCustomerSalesChannelNavigation
{
    ...
    public function handle(CustomerSalesChannel $customerSalesChannel): array
    {
        ...
        if ($isManual || $customerSalesChannel->can_connect_to_platform) {
            $platformNavigation['portfolios'] = [
                ...
                'right_label' => [
                    'number'        => $customerSalesChannel->number_portfolios - ($customerSalesChannel->number_inactive_portfolios ?? $customerSalesChannel->portfolios()->where('status', false)->count()),
                    'class'        => 'text-white',
                ],
