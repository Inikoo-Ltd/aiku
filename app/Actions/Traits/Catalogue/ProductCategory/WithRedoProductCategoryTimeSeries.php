...
trait WithRedoProductCategoryTimeSeries
{
    public function handle(ProductCategory $productCategory, array $frequencies, Command $command = null, bool $async = true): void
    {
        ...
        if (!$productCategory->created_at || $firstInvoicedDate < $productCategory->created_at) {
        ...
        if ($from != null && $to != null) {
            foreach ($frequencies as $frequency) {
                if ($async) {
                    ProductCategoryHydrateTimeSeriesRecords::dispatch($productCategory->id, $frequency, $from, $to)->onQueue('low-priority');
                } else {
                    ProductCategoryHydrateTimeSeriesRecords::run($productCategory->id, $frequency, $from, $to);
                }
            }
        }
    }
