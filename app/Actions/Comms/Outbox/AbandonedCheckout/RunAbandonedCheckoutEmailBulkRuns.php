...
class RunAbandonedCheckoutEmailBulkRuns
{
    use AsAction;

    public string $commandSignature = 'run:abandoned-checkout-notification';
    public string $jobQueue = 'ses';

    public function handle(): void
    {
        $queryOutbox = QueryBuilder::for(Outbox::class);
        $queryOutbox->where('code', OutboxCodeEnum::ABANDONED_CHECKOUT);
        ...
        foreach ($outboxes as $outbox) {
            ProcessAbandonedCheckoutPerOutbox::dispatch($outbox);
        }
    }
