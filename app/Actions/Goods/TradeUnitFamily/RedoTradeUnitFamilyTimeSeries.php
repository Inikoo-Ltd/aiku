...
class RedoTradeUnitFamilyTimeSeries implements ShouldBeUnique
{
    ...
    protected function dateRangeSources(): array
    {
        return [
            [
                ...
                'key'   => 'invoice_transaction_has_trade_units.trade_unit_family_id',
    ...
    public function handle(?int $tradeUnitFamilyId, ?string $from = null, ?string $to = null, bool $async = false): void
    {
        ...
        if (!$from || !$to) {
            $dateRange = $this->getDateRange($tradeUnitFamily->id);
        ...
        foreach (TimeSeriesFrequencyEnum::cases() as $frequency) {
            ...
            if ($async) {
                ProcessTradeUnitFamilyTimeSeriesRecords::dispatch($tradeUnitFamily->id, $frequency, $periodFrom, $periodTo)->onQueue('sales_slave_historic');
            } else {
                ProcessTradeUnitFamilyTimeSeriesRecords::run($tradeUnitFamily->id, $frequency, $periodFrom, $periodTo);
            }
        }
    }
