...
class TradeUnitsHydrateProducts implements ShouldBeUnique
{
    use AsAction;
    use WithEnumStats;
    ...
    public function handle(TradeUnit $tradeUnit): void
    {
        ...
        $stats = [
            'number_products' => $tradeUnit->products()->where('is_main', true)->whereNull('exclusive_for_customer_id')->count()
        ];
