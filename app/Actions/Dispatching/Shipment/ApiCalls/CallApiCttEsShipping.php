...
class CallApiCttEsShipping extends OrgAction
{
    ...
    private function fetchAccessToken(Shipper $shipper): array
    {
        if (app()->environment('production')) {
            ...
            $scope        = Arr::get($shipper->settings, 'scope', 'urn:com:ctt-express:integration-clients:scopes:common/ALL');
        } else {
            ...
            $scope        = config('app.sandbox.shipper_ctt_token.scope', 'urn:com:ctt-express:integration-clients:scopes:common/ALL');
        }
    ...
    public function handle(DeliveryNote $deliveryNote, Shipper $shipper): array
    {
        ...
        $parentResource = GetShippingDeliveryNoteData::run($deliveryNote);
        ...
        $customerReference = Arr::get($parentResource, 'customer_reference', $deliveryNote->reference).'xx';
        if (!app()->environment('production')) {
            $customerReference .= '_'.now()->timestamp;
        }
        ...
        $params = [
            ...
            'delivery'      => [
                ...
                'comments'     => Str::limit(strip_tags(($deliveryNote->shipping_notes ?? ''))),
            ],
            'items'         => $items,
        ];
