...
class IndexPackerPerformanceReport extends OrgAction
{
    private int $records;
    ...
    public function handle(Organisation $organisation, $prefix = null, $dateFilter = null): LengthAwarePaginator
    {
        ...
        // Subquery to get the oldest and newest packing dates per packer
        $packingDateRangeSubQuery = DB::table('packings')
            ->select(
                'packings.packer_user_id',
                DB::raw('MIN(DATE(packings.created_at)) as first_packing_date'),
                DB::raw('MAX(DATE(packings.created_at)) as last_packing_date')
            )
            ->join('delivery_notes', 'packings.delivery_note_id', '=', 'delivery_notes.id')
            ->where('delivery_notes.organisation_id', $organisation->id);

        // Subquery for metrics packing
        $packingsSubQuery = DB::table('packings')
            ->select(
                'packings.packer_user_id as packer_id',
                DB::raw('COUNT(DISTINCT packings.delivery_note_id) as deliveries'),
                DB::raw('SUM(packings.quantity) as items'),
                DB::raw("COUNT(DISTINCT CONCAT(packings.delivery_note_id, '-', delivery_note_items.org_stock_id)) as dp")
            )
            ->join('delivery_notes', 'packings.delivery_note_id', '=', 'delivery_notes.id')
            ->join('delivery_note_items', 'packings.delivery_note_item_id', '=', 'delivery_note_items.id')
            ->where('delivery_notes.organisation_id', $organisation->id);

        $this->records = QueryBuilder::for(User::class)
            ->joinSub($packingsSubQuery->clone()->groupBy('packings.packer_user_id'), 'packings_metrics', function ($join) {
                $join->on('users.id', '=', 'packings_metrics.packer_id');
            })
            ->distinct()
        ...
        if ($dateFilter && !empty($dateFilter['created_at'])) {
            ...
            $packingsSubQuery->whereBetween('packings.created_at', [$startDate, $endDate]);
            $packingDateRangeSubQuery->whereBetween('packings.created_at', [$startDate, $endDate]);
        }

        $packingsSubQuery->groupBy('packings.packer_user_id');
        $packingDateRangeSubQuery->groupBy('packings.packer_user_id');

        // Subquery for timesheet - only count working hours from the first to the last packing date
        $timesheetsSubQuery = DB::table('timesheets')
            ...
            ->joinSub($packingDateRangeSubQuery, 'packing_dates', function ($join) {
                $join->on('users.id', '=', 'packing_dates.packer_user_id');
            })
            ->where('timesheets.subject_type', (new Employee())->getMorphClass())
            ->where('timesheets.organisation_id', $organisation->id)
            ->whereRaw('DATE(timesheets.date) BETWEEN packing_dates.first_packing_date AND packing_dates.last_packing_date')
            ->groupBy('timesheets.subject_id');

        $queryBuilder = QueryBuilder::for(User::class)
            ->joinSub($packingsSubQuery, 'packings_metrics', function ($join) {
                $join->on('users.id', '=', 'packings_metrics.packer_id');
            })
            ...
            ->select(
                ...
                DB::raw('SUM(packings_metrics.deliveries) as deliveries'),
                DB::raw('SUM(packings_metrics.items) as items'),
                DB::raw('SUM(packings_metrics.dp) as dp'),
                DB::raw('SUM(COALESCE(timesheets_metrics.hours, 0)) as hours'),
                DB::raw('CASE WHEN (SUM(COALESCE(timesheets_metrics.hours, 0)) > 0) THEN (SUM(packings_metrics.dp) / SUM(COALESCE(timesheets_metrics.hours, 0))) ELSE 0 END as dp_per_hour'),
        ...
        $totalDpQuery = DB::table('packings')
            ->join('delivery_notes', 'packings.delivery_note_id', '=', 'delivery_notes.id')
            ->join('delivery_note_items', 'packings.delivery_note_item_id', '=', 'delivery_note_items.id')
            ->where('delivery_notes.organisation_id', $organisation->id);
        ...
        if ($dateFilter && !empty($dateFilter['created_at'])) {
            ...
            $startDate = Carbon::createFromFormat('Ymd', $start)->format('Y-m-d');
            $endDate = Carbon::createFromFormat('Ymd', $end)->format('Y-m-d');
            $totalDpQuery->whereBetween('packings.created_at', [$startDate, $endDate]);
        }

        $totalDp = $totalDpQuery->distinct()->count(DB::raw("CONCAT(packings.delivery_note_id, '-', delivery_note_items.org_stock_id)"));

        $queryBuilder->addSelect(DB::raw($totalDp > 0 ? "(SUM(packings_metrics.dp) / $totalDp) * 100 as dp_percentage" : '0 as dp_percentage'));
    ...
    public function tableStructure($prefix = null): Closure
    {
        return function (InertiaTable $table) use ($prefix) {
            ...
            $table
                ...
                ->withEmptyState(
                    [
                        'title'       => __('No Packer Performance Data'),
    ...
    public function htmlResponse(LengthAwarePaginator $data, ActionRequest $request): Response
    {
        ...
        return Inertia::render(
            ...
            [
                ...
                'title'       => __('Packer Performance Report'),
                'pageHead'    => [
                    'title' => __('Packer Performance Report'),
                    'icon'  => [
                        'title' => __('Packer Performance'),
        ...
        return array_merge(
            ...
            [
                [
                    ...
                    'simple' => [
                        'icon'  => 'fal fa-box-open',
                        'label' => __('Packer Performance'),
                        'route' => [
                            'name'       => 'grp.org.reports.packer-performance',
