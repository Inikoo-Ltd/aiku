...
class RedoIntrastatExportTimeSeries
{
    use WithHydrateCommand;

    public string $commandSignature = 'intrastat-export:redo_time_series {organisations?*} {--s|slug=} {--f|frequency=all : The frequency for time series (all, daily, weekly, monthly, quarterly, yearly)} {--a|async : Run synchronously}';
    ...
    {
        $dates = DB::table('delivery_notes')
            ->where('organisation_id', $organisation->id)
            ->whereNotNull('dispatched_at')
            ->selectRaw('MIN(dispatched_at) as min_date, MAX(dispatched_at) as max_date')
            ->first();
