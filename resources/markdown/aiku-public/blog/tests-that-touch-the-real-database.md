...
<aside class="tldr"><strong>TL;DR</strong>≈3,300 Pest tests in 126 files against a real PostgreSQL (≈800 tables), restored from a 6 MB seeded dump before each file, ten parallel processes in CI. <code>TEST_TOKEN=&lt;token&gt;</code> gives every person and agent their own database so a dozen runs coexist. Four flakiness patterns, catalogued. Coverage % is not the point; process paths against semi‑real data are. It is a nightmare to maintain and worth it.</aside>
...
<ul>
<li>Bootstrap: <a href="https://github.com/Inikoo-Ltd/aiku/blob/main/tests/Pest.php">tests/Pest.php</a> — <code>loadDB()</code> reads <code>TEST_TOKEN</code>, restores into <code>aiku_test_&lt;token&gt;</code>, <code>putenv(DB_DATABASE)</code> before boot.</li>
<li>Restore: <a href="https://github.com/Inikoo-Ltd/aiku/blob/main/devops/devel/reset_test_database.sh">devops/devel/reset_test_database.sh</a> — <code>pg_restore -j N</code> then sequences reset to <code>max(id)</code>; dump regenerated by <code>generate_testing_db_dumps.sh</code>.</li>
<li>CI: <a href="https://github.com/Inikoo-Ltd/aiku/blob/main/.github/workflows/testing.yml">testing.yml</a> — parallel run, then <code>git diff --name-only … tests/Feature</code> files re‑run in isolation, twice.</li>
<li>Rules: tokens lowercase alphanumeric; never mix a Unit file and a Feature file in one invocation; always <code>-d xdebug.mode=off</code>.</li>
</ul></aside>
...
<aside class="tldr bottom"><strong>In one paragraph</strong>Test against the real schema, restored fresh per file, in parallel, one database per token; assert deltas not absolutes; constrain your fixtures; and accept the maintenance cost because it is the only thing that catches the bug that moves money.</aside>
