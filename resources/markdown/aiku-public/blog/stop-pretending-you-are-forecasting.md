...
<aside class="tldr"><strong>TL;DR</strong>No forecaster predicts the future; it predicts the past continuing. Ours makes that bet as honest as possible: every SKU in every warehouse carries two live numbers — the day it will run out and how much to reorder now — computed from demand measured only over days the item was actually on the shelf, with Croston or Holt chosen by the shape of the demand, sister organisations filling the silence, no seasonality because it failed the backtest, and a pessimistic P90 date sitting next to the expected one to say how wrong the bet might be. Stored by a hydrator that fires whenever stock moves — no nightly batch, no on-page computation.</aside>
...
- **No local history at all?** Borrow. First the *same stock* sold by sister organisations, damped to half — their market rhymes with ours, it isn't ours; then the average of its product family here, damped harder; and if even that is silent, no forecast rather than an invented one. A new SKU starts life with an informed guess instead of a shrug.

Two things are missing on purpose. There is no **seasonality factor** — what the coming quarter did last year relative to a normal quarter — and no fallback to the item's own older quarterly history. Both used to be there, both were backtested against real dispatches, and both lost. The per-SKU seasonal factor made forecasts worse in 19 of the 24 slices we cut the backtest into: most SKUs sell too few units for one year's quarter to say anything about the next, so the factor mostly amplified noise. The old-history fallback over-forecast by 1.25–1.85×: an item that has gone quiet usually stays quiet. Peaks the data cannot see reliably are left to the buyer, and the buyer's docs say so.

## Two numbers, not a dashboard

The output is deliberately small, per SKU:

**Days of cover** — stock on hand divided by the forecast rate, plus the date that lands on. And because averages are polite fictions, a pessimistic twin: solving *qty = d·μ + 1.28·σ·√d* gives the day the 90th-percentile demand path empties the shelf. If the two dates are far apart, the item is volatile and the earlier one is the one to respect.
...
<li>Fallback when there are no in-stock sales in the 91-day window: same stock in sister organisations ×0.5 → same org stock family ×0.25 → no forecast.</li>
<li>No seasonality, no own-history fallback: a clamped same-quarter-last-year factor and a quarterly-time-series fallback were both backtested on real dispatches and removed — the factor was worse in 19 of 24 slices, the fallback over-forecast 1.25–1.85×.</li>
...
Every number here is a bet that next quarter rhymes with the last one, and every design choice is a hedge for when it doesn't: the damped trend that refuses to believe growth is exponential, the seasonality factor we measured, found wanting and removed rather than let one strange Christmas write next year's plan, the pessimistic P90 date sitting next to the expected one precisely because the expected one will sometimes be wrong. The honest claim is smaller and more useful than prophecy: *if tomorrow behaves like the evidence, this shelf is empty on the 14th — and if tomorrow misbehaves in the usual ways, the 8th.* That is not the future foretold; it is uncertainty measured, which is the only kind of fortune-telling a warehouse can bank on.

<aside class="tldr bottom"><strong>In one paragraph</strong>A forecast is only as honest as its denominator. Count the days the shelf was actually stocked, choose the estimator by the shape of the demand, let your sister companies fill the silence, drop whatever fails the backtest, and write the answer where a page can read it in one query.</aside>
