...
import { blueprint, defaultData } from "@/Components/Websites/Announcement/Templates/Promo/AnnouncementPromo2Countdown/Blueprint"
...
const accordionKey = (index: number) => {
    if (Array.isArray(blueprint[index]?.key)) {
        return blueprint[index]?.key.join('-')
    }

    return blueprint[index]?.key
}

const openFieldWorkshop = inject('openFieldWorkshop', ref<number | null>(null))
const onClickOpenFieldWorkshop = (index?: number | string) => {
    if (openFieldWorkshop && index) {
        openFieldWorkshop.value = index as number
    }
}
