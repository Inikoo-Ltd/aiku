...
import { create, debounce, get, set } from 'lodash-es'
...
<template>
    <div>
        <div v-if="modelValue && modelValue.length" class="mb-4">
            ...
            <div class="border rounded-md overflow-hidden">
                <div v-for="item in modelValue" :key="item.id"
                    ...
                    <!-- Right: quantity editor -->
                    <NumberWithButtonSave :modelValue="get(item, 'quantity_selected', 1)" :bindToTarget="{ min: 1 }"
                        @update:modelValue="(e: number) => set(item, 'quantity_selected', e)" noUndoButton noSaveButton
                        parentClass="w-min" />
                </div>
            </div>
        </div>
        ...
        <!-- Trigger button -->
        <Button @click="showDialog = true" :label="'Open Portfolio Selector'" full type="create"></Button>
                                                ...
                                                <div class="flex flex-col justify-between">
                                                    <div class="w-fit">
                                                        <div v-tooltip="trans('Name')"
                                                            class="w-fit font-semibold leading-none mb-1">{{ item.name
                                                            || 'no name' }}</div>
                                                        <div v-if="!item.no_code" v-tooltip="trans('Code')"
                                                            class="w-fit text-xs text-gray-400 italic">{{ item.code ||
                                                            'no code' }}</div>
                                                        <div v-if="item.reference" v-tooltip="trans('Reference')"
                                                            class="w-fit text-xs text-gray-400 italic">{{ item.reference
                                                            || 'no reference' }}</div>
                                                        <div v-if="item.gross_weight" v-tooltip="trans('Weight')"
                                                            class="w-fit text-xs text-gray-400 italic">{{
                                                            item.gross_weight }}</div>
                                                    </div>
                                                    <div v-if="!item.no_price && item.price" v-tooltip="trans('Price')"
                                                    ...
                                                    <NumberWithButtonSave v-if="withQuantity"
                                                        ...
                                                        :bindToTarget="{ min: 1 }"
                                                        @update:modelValue="(e: number) => (set(item, 'quantity_selected', e), selectedProduct.find(p => p.id === item.id) ? '' : selectedProduct.push(item))"
