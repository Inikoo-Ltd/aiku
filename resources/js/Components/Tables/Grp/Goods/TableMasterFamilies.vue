...
function familyRoute(masterFamily: MasterFamily) {
    ...
    } else if (routeCurrent == "grp.masters.master_shops.show.master_family.missing_image.index") {
        return route(
    ...
    <Table @onCheckedAll="onCheckedAll" :resource="data" :name="tab" class="mt-5" :isCheckBox="isCheckBox"
        ...
        <template #cell(sales_grp_currency_external_delta)="{ item }">
            <div v-if="item.sales_grp_currency_external_delta">
                ...
                <FontAwesomeIcon :icon="getIntervalChangesIcon(item.sales_grp_currency_external_delta.is_positive)?.icon"
