...
<template>
    <div class="relative isolate overflow-hidden">
        ...
        <div class="mx-auto px-12 pb-12 pt-10 lg:flex lg:px-14 ">
            <div v-if="false" xv-if="data.channels.length" class="w-full lg:shrink-0">
                ...
                <div class="flex gapx8">
                    ...
                    <div class="w-full max-w-96 mt-4 xmd:grid grid-cols-1 gap-2 lg:gap-5 xsm:grid-cols-2">
                        <Link
                            ...
                            astart="() => isBoxLoading = true"
                            afinish="() => isBoxLoading = false"
                        >
                            <BackgroundBox xv-if="!stat.is_negative" class="-z-10 opacity-80 absolute top-0 right-0" />
                            <FontAwesomeIcon xv-else icon="fad fa-fire-alt" class="text-red-500 -z-10 opacity-40 absolute -bottom-2 -right-5 text-7xl" fixed-width aria-hidden="true" />

                            <div class="truncate text-sm font-medium" xclass="stat.is_negative ? 'text-red-500' : 'text-gray-400'" xstyle="{ color: stat.is_negative ? stat.color : null }">
                                {{ `stat.label` }}
                            </div>

                            <!-- <dd class="mt-1 text-3xl font-semibold tracking-tight flex gap-x-2 items-center tabular-nums">
                                <LoadingIcon xv-if="isBoxLoading" class='text-xl' />
                                <FontAwesomeIcon xv-else-if="typeof stat.icon === 'string'" xicon='stat.icon' class='text-xl' fixed-width aria-hidden='true' />
                                <Icon xv-else-if="stat.icon" xdata="stat.icon" class="text-xl"/>
