    ...
    <div class="relative w-full max-w-xl mx-auto my-4 md:my-8 overflow-hidden">
        <div class="mx-auto max-w-md ">
            ...
            <div class="mt-5 pt-4 border-t border-dashed">
                <div>{{ trans("Need to pay") }}: <span class="font-bold">{{ locale.currencyFormat(currencyCode, Number(props.needToPay).toFixed(2)) }}</span></div>
                ...
                >
                    <div class="flex flex-row items-center justify-between w-full">
                        ...
                        <FontAwesomeIcon
                            ...
                            aria-hidden="true"
                        />
                        <FontAwesomeIcon v-else icon="fal fa-circle" class="text-xl" fixed-width aria-hidden="true" />
                    </div>
                    <p class="text-gray-400 italic text-sm mt-1">
                        {{ ctrans("Estimated cost") }} • {{ locale.currencyFormat(currencyCode, (needToPay * (parseFloat(option.charge) / 100)).toFixed(2)) }}
                    </p>
                </button>
            </div>
