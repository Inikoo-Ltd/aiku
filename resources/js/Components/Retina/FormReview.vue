...
const totalSize = computed(() =>
	(form.images ?? []).reduce((acc, file) => acc + (file?.size ?? 0), 0)
)
...
const normalizedSchema = computed<SchemaItem[]>(() => {
	...
	return items.filter(
		(item, index, self) =>
			index ===
			self.findIndex(
				(x) => String(x.dimension).toLowerCase() === String(item.dimension).toLowerCase()
			)
	)
})
...
const activeRatings = computed(() => {
	...
	normalizedSchema.value.forEach((item) => {
		const dimension = String(item.dimension ?? "").toLowerCase() as keyof typeof ratingKeyMap
	...
	return ["a", "b", "c", "d", "e"].map((key) => grouped.get(key)).filter(Boolean)
})
...
<template>
	<div class="space-y-4">
		<div class="grid gap-4 lg:grid-cols-2 lg:items-start">
				...
				<div class="space-y-2 rounded-2xl border border-gray-200 bg-white p-4">
					...
					<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						<div class="flex flex-wrap gap-2">
							<button type="button" @click="chooseFiles" :disabled="disabled"
					...
					<input ref="fileInputRef" type="file" class="hidden" accept="image/*" multiple :disabled="disabled"
					...
					<div v-if="imagePreviews.length" class="grid gap-3 pt-2 sm:grid-cols-3">
						<div v-for="(item, index) in imagePreviews" :key="item.file.name + item.file.size"
							...
							<img :src="item.objectURL" :alt="item.file.name" class="h-8 w-full object-cover" />
			...
			<div>
				<!-- Default Layout -->
				<template v-if="showAverageReview">
					<div class="flex flex-col gap-1">
						<div
							...
							<div
								class="rating flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3">
								<div>
								...
								<Rating :modelValue="activeRatings.length
										? averageRating || 0
										: form.rating
									" :readonly="activeRatings.length > 0" :disabled="disabled" :cancel="false"
