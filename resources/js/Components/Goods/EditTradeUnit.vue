...
const fetchProductList = async (url?: string) => {
    ...
    const urlToFetch = url || route(props.tag_routes.index_tag.name, props.tag_routes.index_tag.parameters)
...
const formSelectedTags = useForm({
    tags_id: props.tags_selected_id,
})
const onManageTags = () => {
    formSelectedTags.post(
        route(props.tag_routes.attach_tag.name, props.tag_routes.attach_tag.parameters),
        {
            onStart: () => {
                isLoadingCreateTag.value = true
            },
...
watch(() => props.tags_selected_id, (newTags) => {
...
const onEditTag = () => {
    router[props.tag_routes.update_tag.method || 'patch'](
        route(props.tag_routes.update_tag.name, {
            ...props.tag_routes.update_tag.parameters,
...
const onCreateNewBrand = () => {
    router.post(
        route(props.brand_routes.store_brand.name, props.brand_routes.store_brand.parameters),
        {
...
const onAttachBrand = (brandId: number) => {
    router.post(
        route(props.brand_routes.attach_brand.name, props.brand_routes.attach_brand.parameters),
        {
            brand_id: brandId,
        },
        {
                ...
                <template #option="{ option, isSelected, isPointed }">
                    <div class="flex justify-between w-full">
                        ...
                        <div class="flex gap-x-2">
                            ...
                            >
                                <template #default="{ isOpenModal, changeModel }">
                                    <div @click.stop="changeModel" class="cursor-pointer px-1 text-red-400 hover:text-red-600 rounded-xs">
