...
watch(
    () => [modalOpen.value, contactType.value, shopId.value],
    () => {
        if (modalOpen.value && contactType.value === "guest") {
            loadGuests()
        }
    }
)
...
<template>
    <Dialog v-model:visible="modalOpen" modal :header="ctrans('Phone call')"
        :style="{ width: '92vw', maxWidth: '520px' }" :breakpoints="{ '640px': '95vw' }">
        <ConfirmPopup />
        ...
        <div v-if="!activeCall" class="flex flex-col gap-4 py-2">
            ...
            <button type="button" :disabled="busy" @click="onStart"
                class="w-full flex items-center justify-center gap-3 rounded-xl px-4 py-5 text-base font-semibold text-white transition-colors"
                :class="busy ? 'bg-gray-300 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'">
        ...
        <div v-else class="flex flex-col gap-4 py-1">
            <div class="flex items-center justify-between rounded-xl px-4 py-3"
                ...
                <div class="text-2xl font-semibold tabular-nums"
                    :class="isRunningOut ? 'text-amber-800' : 'text-emerald-800'">
                    {{ formattedElapsed }}
                </div>
            </div>
