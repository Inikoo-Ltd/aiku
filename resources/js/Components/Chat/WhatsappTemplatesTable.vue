...
<template>
    <div>
                ...
                <div class="flex items-center justify-end gap-1">
                    ...
                    <ModalConfirmationDelete :routeDelete="{
                        ...
                        :description="template.is_draft
                            ? ctrans('This draft was never sent to Meta, so it only disappears from Aiku.')
                            : ctrans('It is removed from WhatsApp as well, and every language variant goes with it.')"
        ...
        <Dialog :visible="!!editing" @update:visible="editing = null" modal
            :header="ctrans('What do this template\'s variables mean?')" :style="{ width: '32rem' }">
            <p class="text-xs text-gray-500 mb-4">
                {{ ctrans("Once mapped, Aiku fills these from the conversation and agents no longer type them by hand. The template text is untouched, so Meta does not need to review it again.") }}
            </p>
            ...
            <div class="mt-4 rounded-lg bg-gray-50 p-3">
                <div class="text-[11px] font-medium text-gray-500 mb-1">{{ ctrans("Preview with sample values") }}</div>
            ...
            <Message v-if="!isComplete" severity="warn" :closable="false" class="mt-3 text-xs">
                {{ ctrans("Map every variable, otherwise agents keep filling them by hand.") }}
            </Message>
            ...
            <template #footer>
                ...
                <Button :label="ctrans('Save')" :loading="isSaving" :disabled="!isComplete" @click="save" />
            </template>
        </Dialog>
    </div>
</template>
