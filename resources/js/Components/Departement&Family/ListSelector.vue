...
const getProductList = async (url?: string) => {
  ...
  try {
    const urlToFetch = url || route(props.routeFetch.name, {
      ...
      'filter[global]': searchProductKeyword.value,
      index_perPage: 25,
    })
    const response = await axios.get(urlToFetch)
    productList.value = response.data.data
    productMeta.value = response?.data.meta || null
    productLinks.value = response?.data.links || null
  } catch (e) {
    console.error('Error', e)
    notify({
      title: trans("Something went wrong."),
      text: trans("Error while getting the products list."),
      type: "error"
    })
  } finally {
...
const debounceGetProductList = debounce(() => getProductList(), 500)
      ...
      <div class="mb-2 flex justify-between items-center gap-2">
        <div class="w-64">
          <PureInput
            ...
            @update:modelValue="() => debounceGetProductList()"
            :placeholder="trans('Input to search products')"
          />
      ...
      <div class="text-base font-normal">
        <div class="flex justify-between items-center">
          <div class="font-semibold text-lg py-1">
            {{ props.label_result ?? trans("Result") }} ({{ locale?.number(productMeta?.total || 0) }})
          </div>
        </div>
        ...
        <!-- Pagination (Di luar scroll) -->
        <div class="mt-4">
          <Pagination
            v-if="productMeta"
            :on-click="getProductList"
            :has-data="true"
            :meta="productMeta"
            :per-page-options="[]"
          />
