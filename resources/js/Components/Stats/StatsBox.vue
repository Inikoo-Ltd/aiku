...
<template>
    <component :is="stat.route?.name ? Link : 'div'"
        ...
        }" class="block isolate relative overflow-hidden rounded-lg  border px-4 py-5 shadow-xs sm:p-6 sm:pb-3" :class="[
        ...
        <dd class="mt-1 text-3xl font-semibold tracking-tight flex gap-x-2 items-center tabular-nums">
            ...
            <FontAwesomeIcon v-else-if="typeof stat.icon === 'string'" :icon='stat.icon' class='text-xl' fixed-width
                aria-hidden='true' />
        ...
        <component
            ...
            class="text-base rounded-sm group/mr absolute top-6 right-5 px-2 flex gap-x-0.5 items-center font-normal"
        ...
        <div v-if="stat.metas?.length"
            ...
            <component v-for="(meta, idxMeta) in stat.metas" :is="meta.route?.name ? Link : 'div'"
