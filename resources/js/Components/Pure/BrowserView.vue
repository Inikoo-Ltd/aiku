...
import { faGlobe, faEllipsisV, faStar } from "@fal"
...
<template>
	<div class="flex flex-col items-center space-y-4">
		<!-- Browser Container -->
		<div
			class="rounded-xl shadow-xl overflow-hidden bg-white border border-gray-300 transition-all duration-300"
			...
			<!-- Navigation Bar -->
			<div
				class="flex items-center justify-between bg-white px-3 py-2 border-t border-b space-x-2">
				<!-- Nav Controls -->
				...
				<div class="flex-1 min-w-0">
					<div class="flex items-center bg-gray-100 px-4 py-1 rounded-full w-full">
						...
						<span class="truncate text-sm ml-2 text-gray-700">
							...
							<template v-else>
								{{ url?.domain || "www.website.com"
								}}<span class="text-gray-400">/{{ url?.page }}</span>
							</template>
						</span>
						<FontAwesomeIcon
			...
			<div class="relative h-full w-full bg-white overflow-hidden">
				<slot name="page">
					<div
						class="w-full h-full bg-indigo-100 flex items-center justify-center text-gray-600">
