    ...
    <div class="px-4 py-5 md:px-6 space-y-6">
            ...
            <div v-if="overview.channels.length" class="mt-5">
                ...
                <table class="mt-2 w-full text-xs overflow-x-auto">
                    ...
                    <tbody v-for="group in groupedChannels" :key="group.key">
                        ...
                        <tr v-if="group.channels.length > 1" class="border-b border-gray-50 text-gray-600">
                        ...
                        <tr class="text-gray-900 border-t-2 border-gray-400 font-semibold">
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_5.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(channelTotals.revenue, overview.baseline?.revenue ?? 0) }}</span>
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_3.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(channelTotals.orders, overview.baseline?.orders ?? 0) }}</span>
                    ...
                    <tbody v-if="overview.untraced">
                        <tr class="text-gray-600 border-b border-dashed border-gray-300 leading-tight">
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_5.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(knownDirect.revenue, overview.baseline?.revenue ?? 0) }}</span>
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_3.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(knownDirect.orders, overview.baseline?.orders ?? 0) }}</span>
                        ...
                        <tr v-if="overview.before_tracking && (overview.before_tracking.revenue > 0 || overview.before_tracking.orders > 0)" class="text-gray-600 border-b border-dashed border-gray-300 leading-tight">
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_5.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(overview.before_tracking.revenue, overview.baseline?.revenue ?? 0) }}</span>
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_3.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(overview.before_tracking.orders, overview.baseline?.orders ?? 0) }}</span>
                    ...
                    <tbody v-for="group in outOfScopeGroups" :key="group.key">
                        <tr class="text-gray-600 border-b border-dashed border-gray-300 leading-tight">
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_5.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(group.revenue, overview.baseline?.revenue ?? 0) }}</span>
                                    <span>{{ money(group.revenue) }}</span>
                                </span>
                            </td>
                            <td class="text-right px-2 tabular-nums text-gray-300">—</td>
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_3.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(group.orders, overview.baseline?.orders ?? 0) }}</span>
                                    <span>{{ count(group.orders, decimalColumns.orders) }}</span>
                                </span>
                            </td>
                            <td class="text-right pl-2 tabular-nums text-gray-300">—</td>
                        ...
                        <tr v-for="channel in (isOpen(group.key) ? group.channels : [])" :key="channel.name" class="border-b border-gray-50 text-gray-600">
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_5.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(channel.revenue, overview.baseline?.revenue ?? 0) }}</span>
                            ...
                            <td class="text-right px-2 tabular-nums whitespace-nowrap">
                                <span class="inline-grid grid-cols-[2.75rem_3.5rem]">
                                    <span class="font-normal text-gray-400 text-left">{{ pctOf(channel.orders, overview.baseline?.orders ?? 0) }}</span>
        ...
        <div v-if="overview.email" class="rounded-xl ring-1 ring-gray-200 bg-white p-5">
            ...
            <div class="mt-4 flex flex-wrap gap-x-8 gap-y-2">
                ...
                <div>
                    <span class="text-xs text-gray-500">{{ ctrans('Spam rate') }}</span>
                    <div class="text-sm text-gray-800 tabular-nums">{{ rate(overview.email.totals.spam_rate) }}</div>
                </div>
                <div>
                    <span class="text-xs text-gray-500">{{ ctrans('Average click rate') }}</span>
                    <div class="text-sm text-gray-800 tabular-nums">{{ rate(overview.email.totals.click_rate) }}</div>
                </div>
                <div>
                    <span class="text-xs text-gray-500">{{ ctrans('Unsubscribe rate') }}</span>
                    <div class="text-sm text-gray-800 tabular-nums">{{ rate(overview.email.totals.unsubscribe_rate) }}</div>
                </div>
                <div>
                    <span class="text-xs text-gray-500">{{ ctrans('Bounce rate') }}</span>
                    <div class="text-sm text-gray-800 tabular-nums">{{ rate(overview.email.totals.bounce_rate) }}</div>
                </div>
                <div>
            ...
            <div v-if="overview.email.mailshots.length" class="mt-4 overflow-x-auto">
                <table class="w-full text-xs">
                    <thead>
                        <tr class="text-gray-400 border-b border-gray-100">
                            ...
                            <th class="text-right font-normal py-1.5 px-2">{{ ctrans('Spam rate') }}</th>
                            <th class="text-right font-normal py-1.5 px-2">{{ ctrans('Click rate') }}</th>
                            <th class="text-right font-normal py-1.5 px-2">{{ ctrans('Unsub rate') }}</th>
                            <th class="text-right font-normal py-1.5 px-2">{{ ctrans('Bounce rate') }}</th>
                    ...
                    <tbody>
                        <tr v-for="mailshot in overview.email.mailshots" :key="mailshot.id"
                            ...
                            <td class="text-right px-2 tabular-nums">{{ rate(mailshot.spam_rate) }}</td>
                            <td class="text-right px-2 tabular-nums">{{ rate(mailshot.click_rate) }}</td>
                            <td class="text-right px-2 tabular-nums">{{ rate(mailshot.unsubscribe_rate) }}</td>
                            <td class="text-right px-2 tabular-nums">{{ rate(mailshot.bounce_rate) }}</td>
