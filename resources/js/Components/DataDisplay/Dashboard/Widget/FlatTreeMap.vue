            ...
            <li v-for="(node, nodeIdx) in widget.nodes" :key="node.name" class="pb-0 relative flex flex-1 items-center">
                ...
                >
                    <div class="flex items-center px-4 text-lg xl:px-6 font-medium gap-x-4" :class="[widget.mode == 'compact' ? 'py-2' : node.sub_data?.length ? 'pt-4 md:pt-0 ' : 'py-4']">
                        <LoadingIcon v-if="isLoading === 'node' + nodeIdx" :size="widget.mode == 'compact' ? undefined : 'lg'" class="shrink-0 text-gray-400" />
                        <FontAwesomeIcon v-else-if="node.icon" :size="widget.mode == 'compact' ? undefined : 'lg'" :icon="node.icon" class="shrink-0 text-gray-400" aria-hidden="true" fixed-width />
