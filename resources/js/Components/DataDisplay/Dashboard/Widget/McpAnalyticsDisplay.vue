    ...
    <div class="bg-white rounded-lg p-4 flex flex-col shadow-sm border border-gray-300">
        ...
        <template v-if="widget && widget.calls">
            <div class="flex gap-10 mb-4">
                <div>
                    <p class="text-4xl font-bold">{{ widget.calls.toLocaleString() }}</p>
                    <p class="text-sm text-gray-600">{{ ctrans("Queries") }}</p>
                </div>
                <div>
                    <p class="text-4xl font-bold" :class="widget.error_rate > 10 ? 'text-red-500' : ''">{{ widget.error_rate }}%</p>
                    <p class="text-sm text-gray-600">{{ ctrans("Errors") }}</p>
                </div>
                <div>
                    <p class="text-4xl font-bold">{{ widget.users }}</p>
                    <p class="text-sm text-gray-600">{{ ctrans("Users") }}</p>
                </div>
                <div>
                    <p class="text-4xl font-bold">{{ widget.avg_ms.toLocaleString() }}<span class="text-lg text-gray-400">ms</span></p>
            ...
            <div class="grid grid-cols-3 gap-6 text-sm">
                <div>
                    <p class="text-xs text-gray-400 font-medium mb-1">{{ ctrans("Top tools") }}</p>
                    <div class="divide-y divide-gray-100">
                        <div v-for="tool in widget.top_tools" :key="tool.tool" class="flex justify-between gap-2 py-1">
                ...
                <div>
                    <p class="text-xs text-gray-400 font-medium mb-1">{{ ctrans("Tools with errors") }}</p>
                    <div class="divide-y divide-gray-100">
                        <div v-for="tool in errorTools" :key="tool.tool" class="flex justify-between gap-2 py-1">
                            <span class="text-gray-600 truncate min-w-0">{{ tool.tool }}</span>
                ...
                <div>
                    <p class="text-xs text-gray-400 font-medium mb-1">{{ ctrans("Top users") }}</p>
                    <div class="divide-y divide-gray-100">
                        <Link
