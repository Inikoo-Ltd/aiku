...
<template>
	<div class="relative px-4 mt-4">
		<div class="mb-2 flex justify-between gap-x-2">
			...
			<div @click="isSectionVisible = !isSectionVisible" class="p-2">
				<FontAwesomeIcon icon="fal fa-cog" fixed-width aria-hidden="true" class="text-2xl text-indigo-500 cursor-pointer hover:text-indigo-600 transition" />
			</div>

			<!-- Mobile: Interval Dropdown -->
		</div>
		
		<div v-show="isSectionVisible" class="flex flex-wrap justify-between items-center gap-4 lg:gap-8 mb-2">
			...
			<template v-for="setting, indexSetting in settings" :key="indexSetting">
				<div v-if="setting.align !== 'right'" class="flex items-center space-x-4">
					<template v-if="setting.type === 'toggle'">
						<p class="" :class="[ setting.options[0].value === setting.value ? 'font-medium' : 'opacity-50', ]">
						...
						<ToggleSwitch
							...
							v-tooltip="setting.id" />
						<p class="" :class="[ setting.options[1].value === setting.value ? 'font-medium' : 'opacity-50', ]">
