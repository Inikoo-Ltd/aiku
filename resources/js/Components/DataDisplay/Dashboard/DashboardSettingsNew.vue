...
const updateToggle = (key: string, value: string, valLoading: string) => {
	router.patch(
		route("grp.models.profile.update"),
		{
			settings: {
	...
	<div class="relative">
		...
		<div class="pr-12 mb-2">
			<div v-show="isSectionVisible" class="flex flex-wrap justify-between items-center gap-4 lg:gap-8 mb-2">
				<!-- Toggle: Align left -->
				<div v-for="setting, indexSetting in settings.filter((setting) => setting.align != 'right')" class="flex items-center space-x-4">
