...
const openRename = () => {
	...
	startRename(contextMenu.value.block)
}

const startRename = (block: Daum) => {
	renamingId.value = block.id
	renameValue.value = block.name || block.type
	closeContextMenu()
}

const saveRename = (block: Daum) => {
							...
							<select
								...
								@change="
									(event: { target: { value: any } }) =>
										(filterBlock = event.target.value)
								">
							...
							<draggable
								...
								<template #item="{ element, index }">
									<div
										...
										@contextmenu="
											(event: any) => openContextMenu(event, element)
										">
										<div
											...
											<button
												...
												<span class="text-sm font-medium truncate w-full">
													...
													<input
														...
														v-model="renameValue"
														class="w-full text-sm border border-gray-300 rounded px-1 py-0.5"
														autofocus
														@keyup.enter="saveRename(element)"
														@blur="saveRename(element)"
	...
	<div
		...
		<ul>
			<template v-if="contextMenu.block">
				...
				<li
					@click="
						getRenamePermision(contextMenu.block.web_block.layout.data) && openRename()
					"
					:class="[
						'flex items-center gap-2 px-3 py-2',
