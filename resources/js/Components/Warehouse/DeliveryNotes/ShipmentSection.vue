...
const onPrintShipment = async (ship) => {
	...
	try {
		const response = await axios.post(
			route("grp.models.printing.shipment.label", {
				shipment: ship.id,
			})
		)
...
const onOpenModalTrackingNumber = async () => {
	...
	try {
		const xxx = await axios.get(
			route(
				props.shipments_routes.fetch_route.name,
				props.shipments_routes.fetch_route.parameters
			)
		)
	...
	formTrackingNumber
		...
		.post(
			route(props.shipments_routes.submit_route.name, {
				...props.shipments_routes.submit_route.parameters,
			}),
			{
				preserveScroll: true,
				onStart: () => {
...
const onSaveAddress = (submitShipment: Function) => {
	...
	router.patch(
		route(props.shipping_fields_update_route.name, {
			...
			shipper: formTrackingNumber.shipping_id?.id,
		}),
		filterDataAddress,
		{
			preserveScroll: true,
...
const confirmdelete = (event: MouseEvent, shipment) => {
	confirm.require({
		...
		accept: () => {
			...
			router.delete(
				route("grp.models.shipment.delete", {
...
const copyDeliveryAddress = ref(
	props.shipping_fields
		? {
...
<template>
	<div class="flex gap-x-1 py-0.5">
			...
			<ul v-if="shipments.length" class="list-none">
				<li
					...
					<div class="flex flex-col justify-between gap-x-2 relative">
						...
						<div
							v-if="
								shipment.formatted_tracking_urls &&
								shipment.formatted_tracking_urls.length > 0
							">
							<div
								v-for="(
									trackingItem, trackingIndex
								) in shipment.formatted_tracking_urls"
								:key="trackingIndex"
								class="text-sm">
								<a
		...
		<!-- Modal: Shipment -->
		<Modal
			xv-if="['packed', 'finalised', 'dispatched'].includes(delivery_note_state.value)"
			...
			<div>
				...
				<div v-if="selectedShipment === 'preferred_shipping'" class="ml-6 mb-2">
					<div
						v-if="preferredShipper"
						@click="
							() => (
								set(formTrackingNumber, ['errors', 'address'], null),
					...
					<div v-else class="italic text-sm text-gray-500">
						{{ trans("No preferred shipper for this address") }}
					</div>

					<!-- Manual shipper form: tracking number + save -->
					<template v-if="preferredShipper && !preferredShipper.api_shipper && formTrackingNumber.shipping_id?.id == preferredShipper.id">
						<div class="mt-3">
	...
	<Modal
		:isOpen="isModalErrorShipment"
		@onClose="() => (isModalErrorShipment = false)"
			...
			<div class="w-full mt-3">
				...
				<div
					...
					<div v-if="shipping_fields" class="col-span-2 mb-2">
						...
						<PureInput
							v-model="copyDeliveryAddress.company_name"
					...
					<div v-if="shipping_fields" class="col-span-2 mb-2">
						...
						<PureInput
							v-model="copyDeliveryAddress.contact_name"
					...
					<div v-if="shipping_fields" class="col-span-2 mb-2">
						...
						<PureInput
							v-model="copyDeliveryAddress.phone"
							placeholder="Enter phone number" />
					</div>
					...
					<div v-if="shipping_fields" class="col-span-2 mb-2">
						...
						<PureInput
							v-model="copyDeliveryAddress.email"
