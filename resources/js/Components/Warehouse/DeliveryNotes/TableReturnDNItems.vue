...
<template>
    <Table :resource="data" :name="tab" class="mt-5" rowAlignTop>
        ...
        <template #cell(quantity_picked)="{ item: item, proxyItem }">
            ...
            <span v-if="Number(item.quantity_not_picked) > 0" v-tooltip="ctrans('Not picked')"
                class="text-red-500 rounded-sm border-red-400 bg-red-100  border px-1.5 ml-2">
                {{ Number(item.quantity_not_picked) }}
            </span>

            <Link v-if="isEditable && Number(item.quantity_waiting_warehouse) > 0"
                v-tooltip="ctrans('Waiting for warehouse')" :href="routeItemsWaitingWarehouse(item)"
                class="relative text-amber-500 rounded-sm border-amber-400 bg-amber-100  border px-1.5 ml-2">
                {{ Number(item.quantity_waiting_warehouse) }}
                <FontAwesomeIcon icon="fas fa-circle"
                    class="absolute -top-0.5 xright-0.5 text-amber-500 text-[5px] animate-ping" fixed-width
                    aria-hidden="true" />
                <FontAwesomeIcon icon="fas fa-circle" class="absolute -top-0.5 xright-0.5 text-amber-500 text-[5px]"
                    fixed-width aria-hidden="true" />
            </Link>
            <span v-else-if="Number(item.quantity_waiting_warehouse) > 0" v-tooltip="ctrans('Waiting for warehouse')"
                class="relative text-amber-500 rounded-sm border-amber-400 bg-amber-100  border px-1.5 ml-2">
                {{ Number(item.quantity_waiting_warehouse) }}
                <FontAwesomeIcon icon="fas fa-circle"
                    class="absolute -top-0.5 xright-0.5 text-amber-500 text-[5px] animate-ping" fixed-width
            ...
            <Link v-if="Number(item.quantity_waiting_crm) > 0" :href="routeItemsWaitingCrm(item)">
                <span v-tooltip="ctrans('Waiting for customer services')"
        ...
        <template #cell(pickings)="{ item }">
            <div v-if="item.pickings?.length" class="space-y-1">
                <div v-for="picking in item.pickings" :key="picking.id" class="flex gap-x-2 w-fit">
                    ...
                    <div v-if="picking.type === 'pick'" class="flex gap-x-2 items-center flex-wrap">
                        <Link v-if="!!(generateLocationRoute(picking))" :href="generateLocationRoute(picking)"
                            class="secondaryLink">
                            {{ picking.location_code }}
                        </Link>
                        <span v-else>
                        ...
                        <!-- Section: Picking Batch Code -->
                        <button v-if="picking.show_batch_code_ui"
                    ...
                    <div v-if="picking.type === 'magic_pick'"
                    ...
                    <div v-if="isEditable" class="">
                        <ButtonWithLink v-if="item.quantity_picked != 0 || item.quantity_not_picked != 0"
                            v-tooltip="ctrans('Undo pick :qtyPicked items', { qtyPicked: Number(picking.quantity_picked).toString() })"
                            type="negative" :size="screenType != 'mobile' ? 'xxs' : 'md'" icon="fal fa-undo-alt"
            ...
            <div v-if="!isEditable && Number(item.quantity_waiting_warehouse) > 0"
                class="mt-2 xmx-auto w-fit flex gap-x-2">
        ...
        <template #cell(picking_position)="{ item: itemValue, proxyItem }">
            <div class="hidden">
                ...
                <div><span class="bg-yellow-400">itemValue.quantity_to_pick</span>: {{ itemValue.quantity_to_pick }}
                </div>
                ...
                <div><span class="bg-yellow-400">findLocation(itemValue.locations, get(selectedLocationCode,
                        [itemValue.id], null))</span>: {{ findLocation(itemValue.locations, get(selectedLocationCode,
                            [itemValue.id], null)) }}</div>
                <div><span class="bg-yellow-400">itemValue.is_handled</span>: {{ itemValue.is_handled }}</div>
                <div><span class="bg-yellow-400">itemValue.quantity_required</span>: {{ itemValue.quantity_required }}
                </div>
            </div>
            ...
            <div v-if="itemValue.quantity_to_pick > 0">
                <div v-if="findLocation(itemValue.locations, get(selectedLocationCode, [itemValue.id], null))"
                    ...
                    <div class="mb-3 w-full flex justify-between gap-x-6 xitems-center">
                        <!-- Section: Locations -->
                        <LabelPickingLocation :locations="itemValue.locations"
                        ...
                        <div class="flex items-center flex-nowrap gap-x-2">
                            ...
                            <NumberWithButtonSave
                                ...
                                :key="findLocation(itemValue.locations, get(selectedLocationCode, [itemValue.id], null)).location_code"
                                noUndoButton @onError="(error: any) => {
                                ...
                                <template #save="{ isProcessing, isDirty, onSaveViaForm }">
                                    <div class="flex gap-x-8 w-fit">
                                        <ButtonWithLink
                                            ...
                                            :size="screenType != 'mobile' ? 'xs' : 'md'" type="secondary"
                                            :loading="isProcessing" class="py-0"
                            ...
                            <Button v-if="!itemValue.is_handled
                            ...
                            "
                                ...
                                :size="screenType == 'desktop' ? 'sm' : 'lg'" method="post">
                                <template #label>
                                    <span class="flex items-center">
                                        ...
                                        <FontAwesomeIcon icon="fas fa-wand-magic" class="text-yellow-600" fixed-width
                                            aria-hidden="true" />
                                    </span>
                                </template>
                            </Button>
                            ...
                            <template v-if="!itemValue.is_handled">
                                ...
                                <template v-if="allowWaiting">
                                    <!-- Button: Not picked -->
                                    <ButtonWithLink v-if="allowPickerSetNotPicked" type="negative"
                                        iconRight="fal fa-debug" :size="screenType == 'desktop' ? 'sm' : 'lg'"
                                    ...
                                    <Button
                                        ...
                                        type="tertiary" iconRight="fal fa-hourglass-half"
                                        ...
                                        <template #label>
                                            <div>
                                                <FractionDisplay
                                                    v-if="GetQuantityToPickFractional ? GetQuantityToPickFractional(itemValue) : null"
                                                    :fractionData="GetQuantityToPickFractional(itemValue)" />
                                                <span v-else>{{ locale.number(itemValue.quantity_to_pick ?? 0) }}</span>
                                            </div>
                                        </template>
                                ...
                                <!-- Button: Not picked -->
                                <ButtonWithLink v-else type="negative" iconRight="fal fa-debug"
                                    :size="screenType == 'desktop' ? 'sm' : 'lg'"
                                    :routeTarget="itemValue.not_picking_route" :bindToLink="{ preserveScroll: true }"
                                    v-tooltip="trans('Set :numberNotPicked as not picked', { numberNotPicked: locale.number(itemValue.quantity_to_pick) || '0' })">
                                    <template #label>
                ...
                <div v-else class="flex justify-between gap-x-2">
                    ...
                    <div class="flex gap-x-2 gap-y-1 items-center">
                        <Button @click="() => (isModalEPickMagicPlace = true, selectedItemToPickMagicPlace = itemValue)"
                            type="warning" key="4"
                            v-tooltip="trans('Pick :numberNotPicked from magic place', { numberNotPicked: itemValue.quantity_to_pick || '0' })"
                            :size="screenType == 'desktop' ? 'sm' : 'lg'">
                            <template #label>
                        ...
                        <ButtonWithLink type="negative"
                            v-tooltip="trans('Set :numberNotPicked as not picked', { numberNotPicked: itemValue.quantity_to_pick || '0' })"
                            iconRight="fal fa-debug" :label="itemValue.quantity_to_pick.toString() || '0'"
            ...
            <div v-else-if="Number(itemValue.quantity_waiting_warehouse) < 1 && Number(itemValue.quantity_waiting_crm) < 1"
                class="flex justify-between gap-x-2 gap-y-1">
                ...
                <div class="flex gap-x-2 gap-y-1">
                    <ButtonWithLink v-if="!itemValue.is_handled" type="negative"
                        tooltip="No quantity to pick. Click to ignore." xicon="fal fa-debug" label="Click to ignore"
                        :size="screenType == 'desktop' ? 'sm' : 'lg'" :routeTarget="itemValue.not_picking_route"
                        :bindToLink="{ preserveScroll: true }" />
                </div>
            </div>
            ...
            <div v-if="Number(itemValue.quantity_waiting_warehouse) > 0" class="mt-2 mx-auto w-fit flex gap-x-2">
                ...
                <Button @click="isOpenModalUndoWaitingWarehouse = true, selectedItemToUndoWaitingWarehouse = itemValue"
            ...
            <div v-if="Number(itemValue.quantity_waiting_crm) > 0" class="mx-auto w-fit">
                <Link :href="routeItemsWaitingCrm(itemValue)" class="hover:underline">
                    <LabelItemsWaitingForCrm v-if="Number(itemValue.quantity_waiting_crm) > 0"
    ...
    <Modal :isOpen="isModalPackingInfo" @onClose="closePackingModal" width="w-full max-w-4xl">
        ...
        <Table :resource="modalResource" :name="tab" class="mt-5" rowAlignTop>
            ...
            <template #cell(action)="{ item: item }">
                ...
                <!-- icon="fal fa-debug" -->
                <ButtonWithLink type="negative" tooltip="No quantity to pick. Click to ignore." label="Click to ignore"
                    :size="screenType == 'desktop' ? 'sm' : 'lg'" :routeTarget="item.not_picking_route"
                    :bindToLink="{ preserveScroll: true }" @success="closePackingModal" />

            </template>
        </Table>
        <Button class="mt-6" full label="Close" @click="closePackingModal" />
    </Modal>
    ...
    <Modal :isOpen="isModalLocation" @onClose="isModalLocation = false" width="w-full max-w-3xl" xdialogStyle="{ background: '#ffffff' }">
        <SelectPickingLocation :item="selectedItemValue"
    ...
    <Modal :isOpen="isModalEditExpiryDate" @onClose="() => onCloseModalExpiryDate()" width="w-full max-w-lg">
        <div class="text-center mb-4">
            <div class="font-semibold text-2xl">{{ trans('Batch Code for') }} {{
                selectedItemToEditExpiryDate?.org_stock_code }}:</div>
        ...
        <div class="flex flex-col items-center gap-4">
            <div class="w-full">
                ...
                <PureMultiselectInfiniteScroll v-if="batchCodeFetchRoute" v-model="selectedBatchCode"
                    :fetchRoute="batchCodeFetchRoute" :initOptions="selectedBatchCode ? [selectedBatchCode] : []"
                    labelProp="label" valueProp="id" object :placeholder="trans('Search batch code...')"
                    :disabled="isLoadingSubmitExpiryDate" />
            </div>

            <div class="w-full flex gap-4 mt-4">
                <Button type="negative" size="md" :disabled="isLoadingSubmitExpiryDate" icon="far fa-arrow-left"
                    @click="onCloseModalExpiryDate" :label="trans('Cancel')" />

                <Button type="primary" size="md" :loading="isLoadingSubmitExpiryDate" icon="fad fa-save"
                    @click="onSubmitEditExpiryDate" full :label="trans('Save')" />
            </div>
        </div>
    </Modal>
    ...
    <Modal :isOpen="isModalEPickMagicPlace" @onClose="() => onCloseModalPickMagicPlace()" width="w-full max-w-lg">
        <div class="relative text-left sm:w-full sm:max-w-lg py-2">

            <div class="sm:flex sm:items-start">
                <div
                    class="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-amber-100 sm:mx-0 sm:size-10">
                    <FontAwesomeIcon icon="fal fa-exclamation-triangle" class="text-amber-600" fixed-width
                        aria-hidden="true" />
                </div>
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div class="mt-5 flex flex-row-reverse gap-2">
                        <div class="xw-full sm:w-fit">
                            <Button @click="() => onSubmitPickMagicPlace()" type="warning" key="2"
                                :loading="isLoadingSubmitPickMagicPlace" iconRight="fas fa-wand-magic" full>
                                <template #label>
                                    <div class="whitespace-nowrap">
                                        Yes, pick
                                        <FractionDisplay
                                            v-if="GetQuantityToPickFractional(selectedItemToPickMagicPlace)"
                                            :fractionData="GetQuantityToPickFractional(selectedItemToPickMagicPlace)" />
                                        <span v-else>{{ locale.number(selectedItemToPickMagicPlace?.quantity_to_pick ??
                                            0) }}</span>
                                    </div>
                                </template>
                            </Button>
                        </div>
                        <Button type="tertiary" icon="far fa-arrow-left" :disabled="isLoadingSubmitPickMagicPlace"
                            :label="trans('Cancel')" full @click="() => (isModalEPickMagicPlace = false)" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <Modal :isOpen="isOpenModalSetAsWaiting" width="w-full max-w-lg" @close="isOpenModalSetAsWaiting = false">
        ...
        <div class="flex items-center gap-4 mb-2">
            <div
                ...
                <Image v-if="selectedTransactionToSetAsWaiting?.org_stock_image_thumbnail"
                    :src="selectedTransactionToSetAsWaiting.org_stock_image_thumbnail"
                    :alt="selectedTransactionToSetAsWaiting.org_stock_name" />
                <FontAwesomeIcon v-else icon="fal fa-box" class="text-2xl text-gray-400" fixed-width
                    aria-hidden="true" />
            </div>
        ...
        <div class="flex items-center gap-2 mb-6 p-3 rounded-lg bg-amber-50 border border-amber-200">
            ...
            <span class="font-bold text-amber-800">
                ...
                {{ selectedTransactionToSetAsWaiting.quantity_to_pick +
                    Number(selectedTransactionToSetAsWaiting.quantity_waiting_warehouse || 0) }}
            </span>
        </div>
        ...
        <div class="flex gap-2 mt-6">
            <Button @click="() => isOpenModalSetAsWaiting = false" :label="ctrans('Cancel')" type="negative" />
            <Button @click="() => submitTransactionAsWaiting()" :label="trans('Set as waiting')" full
                iconRight="far fa-arrow-right" :loading="isLoadingSetAsWaiting" />
        </div>
    </Modal>
        ...
        <div class="flex min-h-full xitems-end justify-center p-4 text-center items-center sm:py-4">
            ...
            <div class="sm:flex sm:items-start">
                <div
                    ...
                    <FontAwesomeIcon icon="fal fa-exclamation-triangle" class="text-red-600" fixed-width
                        aria-hidden="true" />
                </div>
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div
                        class="mt-4 bg-amber-50 border border-amber-200 rounded flex items-center gap-4 mb-2 py-2 px-3">
                        <div
                            class="shrink-0 size-16 rounded-lg overflow-hidden xbg-gray-100 border border-black/10 flex items-center justify-center">
                            <Image v-if="selectedItemToUndoWaitingWarehouse?.org_stock_image_thumbnail"
                                :src="selectedItemToUndoWaitingWarehouse.org_stock_image_thumbnail"
                                :alt="selectedItemToUndoWaitingWarehouse.org_stock_name" />
                            <FontAwesomeIcon v-else icon="fal fa-image" class="text-2xl text-gray-400" fixed-width
                                aria-hidden="true" />
                        </div>
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="text-sm text-red-500 opacity-75 italic">
                                {{ ctrans("Quantity waiting for warehouse") }}: {{
                                    Number(selectedItemToUndoWaitingWarehouse?.quantity_waiting_warehouse) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 flex flex-row-reverse gap-2">
                        <div class="xw-full sm:w-fit">
                            <Button :loading="isLoadingUndoWaitingWarehouse"
                                @click="() => (onSetItemToUndoWaitingWarehouse())" type="red"
                                xlabel="props.noLabel ?? trans('Delete')" :icon="'far fa-trash-alt'" full
                                :label="ctrans('Yes, undo waiting')" />
                        </div>

                        <Button type="tertiary" icccon="far fa-arrow-left" :label="ctrans('Cancel')" full @click="
    ...
    <Modal :isOpen="isModalPickingBatchCode" @onClose="onCloseModalPickingBatchCode" width="w-full max-w-lg">
        <div class="text-center mb-4">
            ...
            <div class="opacity-80 italic text-sm">
                <span>{{ selectedPickingForBatchCode?.location_code ? ctrans('Location: :loc', {
                    loc:
                        selectedPickingForBatchCode.location_code }) : '' }} || </span>
        ...
        <div class="flex flex-col items-center gap-4">
            <div class="w-full">
                ...
                <PureMultiselectInfiniteScroll v-if="selectedPickingForBatchCode?.batch_codes_fetch_route"
                    v-model="selectedPickingBatchCode" :fetchRoute="selectedPickingForBatchCode.batch_codes_fetch_route"
                    :initOptions="selectedPickingBatchCode ? [selectedPickingBatchCode] : []" labelProp="label"
                    valueProp="id" object :placeholder="trans('Search batch code...')"
                    ...
                    <template #afterlist>
                        <div class="text-center m-2 py-1 cursor-auto text-blue-400 text-sm">
                            ...
                            <Link :href="route('grp.org.warehouses.show.inventory.batch_codes.index', {
                                organisation: route().params.organisation,
                                warehouse: route().params.warehouse,
            ...
            <div class="w-full flex gap-4 mt-4">
                <Button type="negative" size="md" :disabled="isLoadingSubmitPickingBatchCode" icon="far fa-arrow-left"
                    @click="onCloseModalPickingBatchCode" :label="trans('Cancel')" />
                <Button type="primary" size="md" :loading="isLoadingSubmitPickingBatchCode" icon="fad fa-save"
                    @click="onSubmitPickingBatchCode" full :label="trans('Save')" />
            </div>
        </div>
    </Modal>
</template>
