...
// 🔁 CTA Button Trigger
const triggerCTAEdit = () => {
	const key = Blueprint?.blueprint?.[1]?.key?.join("-")
	sendMessageToParent("activeChildBlock", key)
}

// 📸 Image Edit Trigger
const triggerImageEdit = () => {
	const key = Blueprint?.blueprint?.[0]?.key?.join("-")
	sendMessageToParent("activeChildBlock", key)
}
</script>
