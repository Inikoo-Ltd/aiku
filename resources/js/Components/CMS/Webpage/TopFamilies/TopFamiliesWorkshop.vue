...
watch(
	[allItems, () => props.modelValue?.chip, () => props.modelValue?.container, refreshTrigger],
	async () => {
		await nextTick()
		await computeMaxHeight()
	},
...
<template>
  <!-- swiper + view all -->
 <div
		:id="modelValue?.id ? modelValue?.id : 'families-3' + indexBlock"
			...
			<div class="relative flex-1 overflow-hidden group">
				...
				<div class="swiper-mask lg:px-12 px-0">
					<Swiper
						...
						<SwiperSlide
							class="flex !w-[220px]"
							v-if="modelValue?.show_overview_button">
							<LinkIris
								:href="modelValue?.webpage_data?.overview_url"
								type="internal"
