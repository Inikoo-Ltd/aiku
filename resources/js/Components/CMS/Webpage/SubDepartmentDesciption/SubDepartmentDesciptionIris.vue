...
/* import sub_departmentOfferLabelDiscount from '@/Components/Utils/Label/DiscountTemplate/CategoryQuantityOrderedOrderInterval/sub_departmentOfferLabelDiscount.vue'
import sub_departmentOfferLabelGR from '@/Components/Utils/Label/DiscountTemplate/CategoryQuantityOrderedOrderInterval/sub_departmentOfferLabelGR.vue' */
    ...
    <div :style="{...getStyles(layout?.app?.webpage_layout?.container?.properties, screenType), ...getStyles(fieldValue?.container?.properties), width : 'auto' }"  class="py-4 px-[10px] sm:px-[50px]"
      aria-label="sub_department Description Section">
        ...
        <div class="hidden">
          ...
          <pre><span class="bg-yellow-400">offers_data</span>: {{ fieldValue?.sub_department?.offers_data }}</pre>
        </div>
         
        <!-- Offer: list offers -->
        <div v-if="fieldValue?.sub_department?.offers_data?.number_offers && layout.iris.is_logged_in"
            class="flex flex-col md:flex-row gap-x-4 mt-4 gap-y-1 md:gap-y-2 mb-3 offers"
        >

            <!-- <template v-for="(offer, idOffer, offIdx) in fieldValue?.sub_department?.offers_data.offers">
                <sub_departmentOfferLabelGR 
                  v-if="offer.type == 'Category Quantity Ordered Order Interval'" 
                />

                <sub_departmentOfferLabelDiscount
                    :offer="offer"
                />
            </template> -->

                <DiscountByType 
                   :offers_data="fieldValue?.sub_department?.offers_data"
