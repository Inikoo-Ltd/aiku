...
const name = ref(modelValue.value.sub_department.description_title || modelValue.value.sub_department.name)
...
const saveDescription = debounce(async (key: string, value: string) => {
  try {
    const url = route('grp.models.product_category.update', {
      productCategory: modelValue.value.sub_department.id,
    })
...
watch(name, (val) => {
  modelValue.value.sub_department.description_title = val
...
<template>
  <div id="sub_department-1">
    <div :style="{ ...getStyles(layout?.app?.webpage_layout?.container?.properties, screenType), ...getStyles(modelValue?.container?.properties), width: 'auto' }"
      ...
      <!-- Description Title -->
      <input v-model="name" type="text" placeholder="sub_department Title"
      ...
      <EditorV2 v-model="modelValue.sub_department.description" placeholder="sub_department Description"
      ...
      <div class="rounded-lg">
        <transition name="fade">
          <div v-if="showExtra">
            <EditorV2 v-model="modelValue.sub_department.description_extra" placeholder="Extra Description"
