...
.discount-wrapper :deep(.offer-max-discount) {
  @apply bg-[#A80000] border border-red-900 text-gray-100 flex items-center rounded-sm px-2 py-1 text-xs;
}

.discount-wrapper :deep(.discount-percentage) {
  @apply text-white font-bold px-2;
}
...
.discount-wrapper :deep(.discount-content) {
  @apply max-w-[200px] w-full min-w-0;
}

.discount-wrapper :deep(.discount-triggers) {
  @apply block truncate text-xs leading-4 opacity-70;
}
.discount-wrapper :deep(.gr-content) {
  @apply max-w-[200px] text-sm leading-4 opacity-70 whitespace-nowrap overflow-hidden text-ellipsis;
}
