		...
		<div class="mx-auto grid w-full max-w-6xl grid-cols-1 gap-x-16 gap-y-14 px-4 md:grid-cols-2">
			<div class="mx-auto w-full max-w-md">
				...
				<form class="mt-8 space-y-5" @submit.prevent>
					...
					<div v-if="modelValue?.login?.google?.visible !== false" class="space-y-3">
						<div class="text-center text-sm">
							{{ modelValue?.login?.google?.note }}
						</div>

						<div class="relative flex w-full items-center justify-center gap-x-2 rounded-sm border border-gray-700 bg-white px-4 py-2 text-gray-800">
