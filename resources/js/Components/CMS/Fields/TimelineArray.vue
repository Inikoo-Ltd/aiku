...
import TimelineProperty from "@/Components/Workshop/Properties/TimelineProperty.vue";
        ...
        <div v-if="modelValue?.length" v-for="(field, index) in modelValue" :key="index">
            <Disclosure v-slot="{ open }">
                ...
                <DisclosurePanel class="px-4 pb-2 pt-4 text-sm text-gray-500">
                   <TimelineProperty :modelValue="modelValue[index]"  @update:model-value="(data) => onChangeProperty(index, data)"/>
                </DisclosurePanel>
            </Disclosure>
        </div>
    </div> 
