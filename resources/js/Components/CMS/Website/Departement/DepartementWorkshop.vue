<!--
  ...
  const usedTemplates = ref(
      ...
      props.data.web_block_types.data.find((template) => template.code === props.modelValue?.department?.code) :
      props.data.web_block_types.data[0]
  )
  
  const onPickTemplate = (department: any) => {
      isModalOpen.value = false
      usedTemplates.value = department
              ...
              <div class="px-4 py-5 flex-grow">
                  <SideEditor 
                      ...
                      :blueprint="getBlueprint(usedTemplates.code)" 
                       @update:modelValue="(e) =>{onSaveWorkshop(e)}"
