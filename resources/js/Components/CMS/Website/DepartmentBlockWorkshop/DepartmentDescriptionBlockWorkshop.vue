...
const layoutState = ref(
	JSON.parse(JSON.stringify(Object.values(props.data.layout ?? {})[0] ?? null))
)
...
<template>
	<div class="pt-4">
		<div
			class="h-[85vh] grid gap-4 p-3"
			:class="[sidebarOpen ? 'grid-cols-1 lg:grid-cols-12' : 'grid-cols-1']">
			<Transition
			...
			<div
				class="bg-white rounded-xl shadow-md flex flex-col border overflow-hidden"
          ...
          <div v-if="data.layout" ref="rootRef" :class="[
            ...
            iframeClass
					...
					<component
						...
						:is="
							getComponent(blockLayout.code, {
								shop_type: blockLayout?.shopState?.type,
							})
						"
	...
	<Drawer
		...
		<div class="mx-auto">
			<ul class="space-y-3">
				<li
					v-for="(dept, index) in props.data.department.data"
					:key="dept.slug"
