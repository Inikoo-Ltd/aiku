...
defineProps<{
    steps: { key: string, label: string, current: boolean, done?: boolean, route: routeType }[]
}>()
</script>
    ...
    <div v-if="steps?.length" class="flex items-center gap-x-2 text-sm font-normal whitespace-nowrap">
        <template v-for="(step, index) in steps" :key="step.key">
            ...
            <component :is="step.current ? 'span' : Link"
                ...
                :class="step.current ? 'text-indigo-600 font-medium' : step.done ? 'text-green-600 hover:text-green-700' : 'text-gray-400 hover:text-gray-600'">
