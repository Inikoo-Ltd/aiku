            ...
            >
                <div class="flex items-center">
                    ...
                    <div v-if="breadcrumbs.length > 2 && breadcrumbIdx != 0" class="md:hidden flex items-center">
                        <FontAwesomeIcon v-if="breadcrumbIdx !== 0" class="shrink-0 h-3 w-3 mx-3 opacity-50" icon="fa-regular fa-chevron-right" aria-hidden="true" />
                        <span>...</span>
                    </div>
                    ...
                    <template v-if="breadcrumb.type === 'simple'">
                        ...
                        >
                            <Transition name="spin-to-down">
                                <FontAwesomeIcon v-if="breadcrumb.simple?.icon" :class="breadcrumb.simple.label ? 'mr-1' : ''" fixed-width class="shrink-0 h-3.5 w-3.5" :icon="breadcrumb.simple.icon" aria-hidden="true" />
                            </Transition>
                    ...
                    <template v-else-if="breadcrumb.type === 'creatingModel'">
                        <FontAwesomeIcon class="shrink-0 h-3.5 w-3.5 mr-1 text-yellow-500 ml-2" icon="fas fa-sparkles" aria-hidden="true" />
                    ...
                    <template v-else-if="breadcrumb.type === 'modelWithIndex'">
                        <div class="hidden md:inline-flex">
                            <FontAwesomeIcon v-if="breadcrumbIdx !== 0" class="shrink-0 h-3 w-3 mx-3 opacity-50 place-self-center" icon="fa-regular fa-chevron-right" aria-hidden="true" />
                            <component :is="breadcrumb.modelWithIndex?.index?.url || breadcrumb.modelWithIndex?.index?.route?.name ? Link : 'div'"  class="hover:text-gray-700 grid grid-flow-col items-center"
                                ...
                                <FontAwesomeIcon icon="fal fa-bars" class="shrink-0 h-3.5 w-3.5 mr-1" aria-hidden="true" />
            ...
            <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems
                    ...
                    <template v-if="breadcrumb.type === 'simple'">
                        ...
                        >
                            <!-- Icon Section -->
                            <FontAwesomeIcon v-if="breadcrumb.simple.icon && breadcrumbIdx == 0" class="shrink-0 h-3.5 w-3.5" :icon="breadcrumb.simple.icon" aria-hidden="true" />

                            <!-- Icon Arrow -->
                            <FontAwesomeIcon v-if="breadcrumbIdx != 0" class="shrink-0 h-3.5 w-3.5 text-gray-300" icon="fa fa-arrow-from-left" aria-hidden="true" />
                    ...
                    <template v-else-if="breadcrumb.type === 'modelWithIndex'">
                        <div class="divide-y divide-gray-200">
                            <component :is="breadcrumb.modelWithIndex?.index?.url || breadcrumb.modelWithIndex?.index?.route?.name ? Link : 'div'" class="py-2 grid grid-flow-col justify-start items-center"
                                ...
                                :style="{ paddingLeft: 12 + breadcrumbIdx * 7 + 'px' }"
                            >
                                <FontAwesomeIcon class="shrink-0 h-3.5 w-3.5 text-gray-300" icon="fa fa-arrow-from-left" aria-hidden="true" />
                                <span class="md:text-xs ml-4 mr-3">
                                    {{ breadcrumb.modelWithIndex.index.label }}
                                </span>

                                <!-- Icon List -->
                                <FontAwesomeIcon :icon="['fal', 'bars']" class="shrink-0 h-3.5 w-3.5" aria-hidden="true" />
                            </component>
                            ...
                            >
                                <FontAwesomeIcon class="shrink-0 h-3.5 w-3.5 mr-1 text-gray-300" icon="fa fa-arrow-from-left" aria-hidden="true" />
