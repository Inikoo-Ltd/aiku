...
<template>
    <div class="mx-4 mt-4 flex flex-wrap items-center gap-2 text-sm">
        <span class="flex items-center gap-1.5 text-gray-500">
            ...
            {{ ctrans("Usually made by") }}
        </span>
        <span v-if="!data.current.length" class="text-gray-400">{{ ctrans("nobody yet") }}</span>
        <span
            ...
            {{ artisan.name }}
            <button v-if="data.detach_route" type="button" class="text-gray-400 hover:text-red-600" :title="ctrans('Remove')" @click="detach(artisan)">
        ...
        <select v-if="data.attach_route && available.length" v-model="adding" class="rounded border-gray-300 py-0.5 text-xs" @change="attach">
            <option value="">{{ ctrans("Add artisan") }}…</option>
