<template>
    <nav v-if="!hasData || pagination?.total < 1 || exportLinks?.export?.route || hasPagination"
        class="bg-white px-4 py-3 border-t border-gray-200 sm:px-4"
        :class="customWrapperClass">
        <slot name="topRow" :data="{ paginated: meta?.total > 15}">
        </slot>
