...
type ShopEntry = {
    ...
    name: string
    code: string
    needs_review_items: { count: number; route: { name: string; parameters: Record<string, string> } }
}
...
watch(() => props.open, async (isOpen) => {
    ...
    try {
        const response = await axios.get(route('grp.json.products_need_review_badge'))
        data.value = response.data
        data.value.forEach((org: OrgEntry) => expandedOrgs.value.add(org.organisation.slug))

        const layout = useLayoutStore()
        layout.products_need_review_count = data.value.reduce((total, org) =>
            total + org.shops.reduce((sTotal, s) =>
                sTotal + s.needs_review_items.count, 0
            ), 0
        )
    } finally {
        ...
        <div v-else>
            <div v-for="orgData in data" :key="orgData.organisation.slug" class="mb-2">
                ...
                <div v-if="expandedOrgs.has(orgData.organisation.slug)" class="pl-3 mt-1 space-y-1">
                    <div v-for="shop in orgData.shops" :key="shop.slug" class="space-y-1" :class="shop.needs_review_items.count > 0 ? '' : 'hidden'">
                        <p class="text-xs text-gray-400 font-medium">{{ shop.name }}</p>

                        <Link
                            v-if="shop.needs_review_items.count > 0"
                            :href="route(shop.needs_review_items.route.name, shop.needs_review_items.route.parameters)"
                            @click="close()"
                            class="flex items-center justify-between px-2 py-1 rounded hover:bg-emerald-50 group"
                        >
