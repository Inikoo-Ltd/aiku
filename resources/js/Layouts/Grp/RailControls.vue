...
<template>
    <div
        ...
        :class="layout.messagingSidebar.show ? 'flex flex-wrap items-center gap-2 px-2 py-2' : 'flex flex-col items-center gap-y-2 py-2'">
        <!-- Button: Profile -->
        ...
        <div v-if="layout?.dispatching_waiting_count > 0" class="relative flex items-center justify-center shrink-0" :class="layout.messagingSidebar.show ? '' : 'h-9 w-9'">
            <Popover width="w-80" position="right-full mr-2 top-0">
                <template #button="{ open }">
                    <div class="relative bg-amber-300 text-amber-700 rounded w-9 h-7 flex items-center justify-center opacity-70 hover:opacity-100 cursor-pointer font-semibold tabular-nums">
                        <Transition name="spin-to-right"><span :key="layout?.dispatching_waiting_count"><span :class="layout?.dispatching_waiting_count > 99 ? 'text-xxs' : 'text-sm'">{{ layout?.dispatching_waiting_count > 99 ? '99+' : layout?.dispatching_waiting_count }}</span></span></Transition>
        ...
        <div v-if="layout?.crm_waiting_count > 0" class="relative flex items-center justify-center shrink-0" :class="layout.messagingSidebar.show ? '' : 'h-9 w-9'">
            <Popover width="w-80" position="right-full mr-2 top-0">
                <template #button="{ open }">
                    <div class="relative bg-purple-300 text-purple-700 rounded w-9 h-7 flex items-center justify-center opacity-70 hover:opacity-100 cursor-pointer font-semibold tabular-nums">
                        <Transition name="spin-to-right"><span :key="layout?.crm_waiting_count"><span :class="layout?.crm_waiting_count > 99 ? 'text-xxs' : 'text-sm'">{{ layout?.crm_waiting_count > 99 ? '99+' : layout?.crm_waiting_count }}</span></span></Transition>
        ...
        <div v-if="layout?.crm_return_count > 0" class="relative flex items-center justify-center shrink-0" :class="layout.messagingSidebar.show ? '' : 'h-9 w-9'">
            <Popover width="w-80" position="right-full mr-2 top-0">
                <template #button="{ open }">
                    <div class="relative bg-blue-300 text-blue-700 rounded w-9 h-7 flex items-center justify-center opacity-70 hover:opacity-100 cursor-pointer font-semibold tabular-nums">
                        <Transition name="spin-to-right"><span :key="layout?.crm_return_count"><span :class="layout?.crm_return_count > 99 ? 'text-xxs' : 'text-sm'">{{ layout?.crm_return_count > 99 ? '99+' : layout?.crm_return_count }}</span></span></Transition>
        ...
        <div v-if="layout?.master_updated_count > 0" class="relative flex items-center justify-center shrink-0" :class="layout.messagingSidebar.show ? '' : 'h-9 w-9'">
            <Popover width="w-80" position="right-full mr-2 top-0">
                <template #button="{ open }">
                    <div class="relative bg-rose-300 text-rose-700 rounded w-9 h-7 flex items-center justify-center opacity-70 hover:opacity-100 cursor-pointer font-semibold tabular-nums">
                        <Transition name="spin-to-right"><span :key="layout?.master_updated_count"><span :class="layout?.master_updated_count > 99 ? 'text-xxs' : 'text-sm'">{{ layout?.master_updated_count > 99 ? '99+' : layout?.master_updated_count }}</span></span></Transition>
