...
const onSubmitWoocommerce = async () => {
	try {
		...
		const response = await axios.post(
			route(
				props.type_woocommerce?.connectRoute?.name,
				props.type_woocommerce.connectRoute.parameters
			),
...
const onSubmitManualKeys = async () => {
	try {
		...
		await axios.post(route("retina.models.dropshipping.woocommerce.tmp_user.store"), {
			...manualKeys.data(),
			...(wooCommerceInput.url
				? { url: wooCommerceInput.url.trim().replace(/\/+$/, "") }
				: {}),
		})
		await submitForm()
	} catch (err: any) {
...
const submitForm = async () => {
	...
	try {
		const { data } = await axios.get(
			route("retina.models.dropshipping.woocommerce.tmp_user_keys_check")
		)
		goNext()
		isLoadingStep.value = false
	} catch (err: any) {
...
<template>
	<div class="flex flex-col gap-2">
		...
		<span class="text-sm">{{
			trans("This is where you need to auth your store to our system.")
		}}</span>
	</div>
	<form @submit.prevent="submitForm" class="flex flex-col gap-6">
		<div class="flex items-center gap-2 w-full md:w-80">
			<PureInputWithAddOn
				v-model="wooCommerceInput.url"
				:leftAddOn="{
					icon: 'fal fa-globe',
				}"
				:placeholder="'e.g https://storeurlexample.com'"
				@keydown.enter="() => onSubmitWoocommerce()" />
			<Button size="sm" :loading="isLoadingStep" @click="onSubmitWoocommerce">{{
				trans("Auth Store")
			}}</Button>
			<FontAwesomeIcon
				v-tooltip="
					trans(
						'Requests a token from Woocommerce so we can sync without you entering your account details each time'
					)
				"
		...
		<button
			type="button"
			class="text-sm text-left underline text-gray-600 w-fit"
			@click="showManualKeys = !showManualKeys">
		...
		<div v-if="showManualKeys" class="flex flex-col gap-2 w-full md:w-96">
			<p class="text-sm text-gray-600">
				{{
					trans(
						"In WooCommerce go to Settings, Advanced, REST API and create a key with Read/Write permissions, then paste it here."
					)
				}}
			</p>
			<PureInputWithAddOn
				v-model="manualKeys.consumer_key"
				:leftAddOn="{ icon: 'fal fa-key' }"
				:placeholder="'ck_...'" />
			<PureInputWithAddOn
				v-model="manualKeys.consumer_secret"
				:leftAddOn="{ icon: 'fal fa-lock' }"
				:placeholder="'cs_...'" />
			<Button
				size="sm"
				:loading="isLoadingStep"
				:disabled="!manualKeys.consumer_key || !manualKeys.consumer_secret"
				@click="onSubmitManualKeys"
				>{{ trans("Use these keys") }}</Button
			>
		</div>
		...
		<div class="flex md:justify-end gap-4">
			<Button type="secondary" size="sm" @click="closeCreateWooModal">{{
				trans("Cancel")
			}}</Button>
