	...
	<div class="p-6 grid grid-cols-2 gap-x-4">
		<div
			...
			<div
				...
				<div class="px-6 text-4xl">
					<FontAwesomeIcon
						v-if="route().params.platform === 'shopify'"
						v-tooltip="'Shopify'"
						:icon="faShopify"
						class=""
						fixed-width
						aria-hidden="true" />
					<FontAwesomeIcon
						v-if="route().params.platform === 'tiktok'"
						v-tooltip="'Tiktok'"
						:icon="faTiktok"
						class=""
						fixed-width
						aria-hidden="true" />
					<svg
						v-if="route().params.platform === 'woocommerce'"
						...
						<g
							...
							stroke="none">
							<path
								d="M483 3336 c-106 -34 -195 -115 -235 -215 l-23 -56 -3 -653 c-2 -423
                        ...
                        47 275 23z" />
							<path
								d="M2500 2683 c-35 -13 -70 -46 -104 -97 -100 -152 -119 -332 -50 -475
                        47 -96 120 -102 212 -18 81 75 126 198 125 342 0 179 -80 287 -183 248z" />
							<path
								d="M3494 2671 c-143 -87 -218 -371 -142 -541 32 -70 57 -92 106 -94 159
			...
			<div class="flex flex-col gap-y-2 w-full py-4 px-5">
				...
				<dl
					...
					<dd class="text-gray-500">
						<time datetime="2023-01-31">{{
							useFormatTime(data?.customer?.created_at)
						}}</time>
					</dd>
				</dl>
				<!-- Field: Email -->
				...
				<!-- Field: Address -->
				<dl
					v-if="data?.customer?.address"
					class="relative flex items w-full flex-none gap-x-4">
		...
		<div>
			<div class="h-fit flex gap-x-2 w-fit text-xs">
				...
				<div
					v-if="
						...
						data?.customer_sales_channel?.exist_in_platform
					"
					class="border border-gray-300 rounded flex items-center gap-x-2 px-2 py-1 w-fit">
					<FontAwesomeIcon
						v-if="data?.customer_sales_channel?.exist_in_platform"
						icon="fal fa-check"
						class="text-green-500"
						fixed-width
						aria-hidden="true" />
				...
				<div
					...
					class="border border-gray-300 rounded flex items-center gap-x-2 px-2 py-1 w-fit">
					<FontAwesomeIcon
						v-if="data?.customer_sales_channel?.platform_status"
						icon="fal fa-check"
						class="text-green-500"
						fixed-width
						aria-hidden="true" />
			...
			<div class="mt-3 h-fit flex gap-x-2 w-fit text-xs">
				...
				<ModalConfirmation
					v-if="
						data?.customer_sales_channel?.status === 'closed' &&
						data?.customer_sales_channel?.platform_id === 3
					"
					:routeYes="{
			...
			<div
				v-if="
					data.fulfilment_policies?.total &&
					data.fulfilment_policies?.fulfillmentPolicies?.length
				"
				...
				<div class="mt-1 grid grid-cols-2 gap-4">
					<div
						v-for="policy in data.fulfilment_policies?.fulfillmentPolicies"
