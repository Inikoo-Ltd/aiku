	...
	formProducts
		...
		.post(
			route(data.route?.name || "#", {
				...
				historicAsset: formProducts.historicAssetId,
			}),
			{
				preserveScroll: true,
				onSuccess: () => {
...
const fetchPaymentMethod = async () => {
	try {
		...
		const { data } = await axios.get(
			route(
				props.box_stats.products.payment.routes.fetch_payment_accounts.name,
				props.box_stats.products.payment.routes.fetch_payment_accounts.parameters
			)
		)
...
<template>
				...
				<ModalProductList
					...
					<template #default="{ isOpenModal, changeModel }">
						<Button
							...
							:label="action.label"
							:icon="action.icon"
							@click="() => { changeModel(); openModal(action); }"
	...
	<!-- Section: Timeline -->
	<div
		v-if="props.data?.data?.state != 'in_process' && currentTab != 'products'"
	...
	<div
		...
		class="grid grid-cols-2 lg:grid-cols-4 divide-x divide-gray-300 border-b border-gray-200">
		<BoxStatPallet class="py-2 px-3" icon="fal fa-user">
			...
			<div
				v-if="
					box_stats?.customer?.addresses?.billing?.formatted_address !==
					box_stats?.customer?.addresses?.delivery?.formatted_address
				"
			...
			<div
				...
				"
				class="mt-2 pl-1 flex items w-full flex-none gap-x-2"
				v-tooltip="trans('Shipping address and Billing address')">
				<dt class="flex-none">
					<FontAwesomeIcon
						icon="fal fa-shipping-fast"
						class="text-gray-400"
		...
		<BoxStatPallet class="col-span-2 border-t lg:border-t-0 border-gray-300">
			<section aria-labelledby="summary-heading" class="rounded-lg px-4 py-4 sm:px-6 lg:mt-0">
				...
				<OrderSummary
					:order_summary="box_stats.order_summary"
				...
				<div class="col-span-2">
					...
					<div class="space-x-1">
						<span class="text-xxs text-gray-500"
							...
							{{
								locale.currencyFormat(
									box_stats.order_summary.currency.code || "usd",
									box_stats.products.payment.pay_amount
								)
							}}</span
						>
						<Button
							...
							"
							:disabled="
								paymentData.payment_amount === box_stats.products.payment.pay_amount
							"
							type="tertiary"
							label="Pay all"
							size="xxs" />
