    ...
    <div class="px-8 py-6">
        <div class="flex justify-between gap-8">
            ...
            <div class="max-w-lg first:pt-0 pr-2 flex flex-col first:border-t-0 gap-y-1 pt-1 pb-1.5">
                ...
                <div v-if="(typeof data.offer.trigger_data.item_quantity !== 'undefined')"
                    class="mb-2 grid grid-cols-7 gap-x-4 items-center justify-between">
                    <dt class="col-span-4 flex flex-col">
                        <div class="flex items-center leading-none">
                            <span>Item quantity</span>
                            <FontAwesomeIcon icon='fal fa-question-circle'
                                v-tooltip="'Minimum of quantity the item ordered'"
                ...
                <!-- Trigger: Item Quantity -->
                <div v-if="(typeof data.offer.trigger_data.min_amount !== 'undefined')"
                    class="mb-2 grid grid-cols-7 gap-x-4 items-center justify-between">
                    <dt class="col-span-4 flex flex-col">
                        <div class="flex items-center leading-none">
                            <span>Order amount</span>
                            <FontAwesomeIcon icon='fal fa-question-circle' v-tooltip="'Minimum of amount of the order'"
                                class='ml-1 cursor-pointer text-gray-400 hover:text-gray-500' fixed-width
                                aria-hidden='true' />
                        </div>
                ...
                <!-- Trigger: Item Quantity -->
                <div v-if="(typeof data.offer.trigger_data.order_number !== 'undefined')"
                    class="mb-2 grid grid-cols-7 gap-x-4 items-center justify-between">
                    <dt class="col-span-4 flex flex-col">
                        <div class="flex items-center leading-none">
                            <span>Minimum order</span>
                            <FontAwesomeIcon icon='fal fa-question-circle'
                                v-tooltip="'Minimum order required to activate the discount'"
                                class='ml-1 cursor-pointer text-gray-400 hover:text-gray-500' fixed-width
                                aria-hidden='true' />
                        </div>
                        <span
                            v-tooltip="`The order count required to activate the discount (e.g., '7' is mean their 7th order)`"
            ...
            <div class="ml-4 max-w-lg first:pt-0 pr-2 flex flex-col first:border-t-0 gap-y-1 pt-1 pb-1.5">
                ...
                <div class="flex flex-col gap-2 w-96">
                    <div v-for="gift in data.products" :key="gift.id" class="flex items-center gap-2">
                        <div class="w-14 aspect-square h-14 border border-gray-300">
                            <Image :src="gift.web_images_main?.thumbnail" :alt="gift.name"
                                class="object-contain w-full h-full" />
                        </div>
                        <label :for="gift.id.toString()" class="">
                            <Link :href="routeProduct(gift)" class="secondaryLink font-bold text-sm">{{ gift.code }}
                            </Link>
                            <span v-if="gift.default"
                                v-tooltip="ctrans(`This product will auto selected in Customer's order`)"
                                class="ml-2 bg-pink-500 text-white text-xs rounded-sm px-1">Default</span>
                            <br />
