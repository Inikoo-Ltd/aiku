...
const loadOptions = async () => {
  const { data } = await axios.get(route('grp.sysadmin.notification-settings.guests.options'))
...
const submit = () => {
  form.post(route('grp.sysadmin.notification-settings.guests.store'), {
    onSuccess: () => {
...
<template>
  <div>
    ...
    <Table :resource="settings" :name="'guestNotificationSettings'">
        ...
        <div class="flex flex-col gap-1">
          ...
          >
            ...
            <ModalConfirmationDelete
              :routeDelete="{
                name: 'grp.sysadmin.notification-settings.guests.delete',
    ...
    <Modal :isOpen="showModal" @onClose="showModal = false" :title="editMode ? 'Edit Guest Notification' : 'Assign Guest Notification'" width="w-full max-w-lg">
      <div class="space-y-4">
        <div>
