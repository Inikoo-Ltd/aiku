...
<template>
					...
					<Menu as="div" class="relative inline-block text-left">
						...
						<transition
							...
							<MenuItems
								...
								:class="
									formData.submitPosition === 'top' ? 'top-full' : 'top-[90px]'
								">
								<div class="px-1 py-1">
									<div v-for="(item, index) in formData.route">
										<MenuItem>
											<div
												class="hover:bg-gray-50 group flex w-full items-center rounded-md px-2 py-2 text-sm cursor-pointer"
	...
	<div class="rounded-lg bg-white shadow-sm">
		<div
			...
			<form
				:class="[
					'px-4 sm:px-6 md:px-10 gap-y-8 pb-8 divide-y divide-gray-200',
				...
				<template
					v-for="(sectionData, sectionIdx) in formData['blueprint']"
					:key="sectionIdx">
					<!-- If Section: all fields is not hidden -->
					<div
						v-if="
							!Object.values(sectionData.fields).every((field: any) => field.hidden)
						"
						class="relative py-4">
						...
						<div class="mt-2 pt-4 sm:pt-5">
							<template
								v-for="(fieldData, fieldName, index) in sectionData.fields"
								:key="index">
								<!-- If Field is not hidden = true -->
								<dl
									v-if="!fieldData.hidden"
									class="mt-1 pb-4 sm:pb-5 sm:grid sm:grid-cols-3 sm:gap-4"
				...
				<!-- Button -->
				<div
					v-if="!formData?.submitPosition || formData?.submitPosition != 'top'"
					...
					<div
						v-else-if="formData.submitButton == 'dropdown'"
