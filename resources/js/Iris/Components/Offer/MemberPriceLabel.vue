    ...
    <div class="inline-flex items-center gap-1 rounded cursor-pointer transition-all duration-150 "
        ...
        <Popover ref="infoPopover">
            <div class="max-w-[280px] space-y-3 text-sm">
                <p v-if="props.isGoldenProduct" class="font-semibold">
                    {{ getGoldenProductTriggerLabel(props.offer) }}
                </p>
                ...
                <div v-if="webpage_data?.sub_type != 'family'">
                    <p class="text-[#555]">
                        {{ ctrans( "This product qualifies for an exclusive Gold Reward member discount." ) }}
                    </p>

                    <p class="text-[#555]">
                        {{ ctrans( "Sign in with your Gold Reward account to access member pricing and enjoy additional savings on  this product." ) }}
                    </p>

                </div>
                <div v-else>
                    <p v-if="props.isGoldenProduct" class="text-[#555] text-justify">
                        {{ ctrans('Add a Golden Product to your basket to unlock the volume discount across the entire product family.') }}
                    </p>
                    <p v-else class="text-[#555]" v-html="offer?.products_triggers_label">
                    </p>
                </div>
