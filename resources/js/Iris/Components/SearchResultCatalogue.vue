    ...
    <div class="grid grid-cols-5 h-full min-h-0 w-full col-span-12">
        ...
        <div class="col-span-2 border-r flex flex-col min-h-0">
            <div class="px-4 pt-4 pb-2 border-b">
                <p class="text-xs font-semibold uppercase tracking-wider text-gray-400">{{ ctrans('Categories & Collections') }}</p>
            </div>
            <div class="flex-1 overflow-y-auto p-3 space-y-1">
                ...
                <template v-else>
                    <template v-if="results?.product_categories?.length">
                        <p class="text-[10px] uppercase tracking-widest text-gray-400 px-2 pt-1 pb-0.5">{{ ctrans('Departments & Families') }}</p>
                        <LinkIris
                            v-for="category in results.product_categories"
                            :key="category.id"
                            :href="category.url"
                            class="group relative flex items-center gap-3 rounded-lg px-2 py-1.5 hover:bg-slate-100 transition cursor-pointer"
                            @success="() => model = false"
                        >
                    ...
                    <template v-if="results?.collections?.length">
                        <p class="text-[10px] uppercase tracking-widest text-gray-400 px-2 pt-3 pb-0.5">{{ ctrans('Collections') }}</p>
                        <LinkIris
        ...
        <div class="col-span-3 flex flex-col min-h-0">
            <div class="px-4 pt-4 pb-2 border-b">
                <p class="text-xs font-semibold uppercase tracking-wider text-gray-400">{{ ctrans('Products') }}</p>
            </div>
                ...
                <template v-if="results?.products?.length">
                    <div class="grid grid-cols-4 gap-2">
                        <LinkIris
                            ...
                            :key="product.id"
                            :href="product.url"
                            class="group relative flex flex-col rounded-lg overflow-hidden border border-transparent hover:border-slate-200 hover:shadow-sm transition cursor-pointer"
                            @success="() => model = false"
                        >
