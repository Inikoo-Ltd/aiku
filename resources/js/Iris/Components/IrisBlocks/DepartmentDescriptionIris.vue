...
<template>
  <div :id="fieldValue?.id ? fieldValue?.id : 'department-1-iris' + indexBlock" component="department-1-iris">
    <div
      :style="{ ...getStyles(layout?.app?.webpage_layout?.container?.properties, screenType), ...getStyles(fieldValue?.container?.properties), width: 'auto' }"
      class="py-4 px-[10px] sm:px-[50px]" aria-label="department Description Section">
      ...
      <div v-if="fieldValue?.department?.offers_data?.number_offers && layout.iris.is_logged_in"
        ...
        <DiscountByType :offers_data="fieldValue?.department?.offers_data"
        ...
        <DiscountByType
          ...
          :offers_data="fieldValue?.department?.offers_data" :template="'triggers_labels'" />
      </div>

      <!-- Description Title (SEO: Heading) -->
      <h1 v-if="fieldValue.department.description_title || fieldValue.department.name"
        class="text-[1.5rem] leading-[2rem] font-semibold">
        {{ fieldValue.department.description_title ?? fieldValue.department.name }}
      </h1>

      <!-- Main Description -->
      <div v-if="cleanedDescription" id="description-department-1-iris" xstyle="{ marginTop: 0 }"
        v-html="cleanedDescription" class="mt-6 text-justify" />
...
.offers :deep(.offer-max-discount) {
  @apply bg-[#A80000] border border-red-900 text-gray-100 w-fit flex items-center rounded-sm px-1 py-0.5 text-xl sm:px-1.5 sm:py-1 sm:text-xl md:px-2 md:py-1;
}
</style>
