...
const allItems = computed(() => [...(props.fieldValue?.families || [])])
...
watch(
	[allItems, () => props.fieldValue?.chip, () => props.fieldValue?.container, refreshTrigger],
	async () => {
		await nextTick()
		await computeMaxHeight()
	},
...
<template>
	<div
		:id="fieldValue?.id ? fieldValue?.id : 'families-3' + indexBlock"
		component="families-3"
		:key="refreshTrigger"
		ref="containerRef" class="pt-5">
		<div
			...
			<div class="relative flex-1 overflow-hidden group">
				...
				<div class="swiper-mask lg:px-12 px-2">
					<Swiper
						...
						<SwiperSlide
							class="flex !w-[220px]"
							v-if="fieldValue?.show_overview_button">
							<LinkIris
								:href="fieldValue?.webpage_data?.overview_url"
								type="internal"
